<template>
  <el-row>
    ...
    <el-dialog :title="dialogUserName" :visible.sync="dialogUserVisible" class="groupUserDialog">
...
<script>
  ...
  import { getAllChildrenIds } from '@/utils/tree';
        ...
        getByGroupId(id).then(response => {
          if (response.data.length > 0) {
            ...
            for (let i in response.data) {
              ids.push(response.data[i].departId);
            }
          ...
          if (valid) {
            putObj(this.form.id, this.form).then(response => {
              if (response.status == 200) {
                if (this.form.dataRange == '0') {
                  this.addGroupOffice(response.data.id, departList);
                }else {
                  this.$notify({
                    title: '成功',
                    message: '修改成功',
