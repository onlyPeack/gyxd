  ...
  <div class="app-container bill-container">
      ...
      <div style="margin-top: 15px">
        <el-form :inline="true" :model="listQuery" size="small" label-width="140px">
          ...
          <el-form-item label="渠道商户ID：">
            <el-input v-model="listQuery.channelMchId" class="input-width" placeholder="渠道商户ID"></el-input>
          </el-form-item>
    ...
    <div class="table-container">
      <el-table ref="homeAdvertiseTable"
        ...
        <el-table-column label="渠道ID" width="120" align="center">
          <template slot-scope="scope">{{scope.row.channelId}}</template>
        </el-table-column>
        <el-table-column label="渠道名称" width="120" align="center">
          <template slot-scope="scope">{{scope.row.channelName}}</template>
        </el-table-column>
        <el-table-column label="渠道商户ID" align="center">
        ...
        <el-table-column label="渠道状态" width="120" align="center">
          <template slot-scope="scope">
            {{scope.row.state==0?'停止使用':'使用中'}}
          </template>
        </el-table-column>
        <el-table-column label="配置参数" width="120" align="center">
...
<script>
  import {fetchList,createHomeCentre,updateHomeCentre,getHomeCentre} from '@/api/mall/pay/homePayChannel';
  ...
  export default {
    ...
    methods: {
      ...
      //   }).then(() => {
      //
      //     updateAuditStatus(row.id, {status: row.status}).then(response => {
      //       this.getList();
      //       this.$message({
      //         type: 'success',
      //         message: '修改成功!'
