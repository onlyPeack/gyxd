<template>
  <div class="app-container calendar-list-container sales-detail-list-container full-purchase bill-container">
    ...
    <el-table size="small" :data="list" v-loading="listLoading" element-loading-text="正在查询中。。。" border fit
      ...
      <el-table-column align="center" label="描述" prop="content" v-slot="{row}" width="1000">
        <el-link :underline="false" type="primary" @click="showFeedbackDetail(row)">{{row.content}}</el-link>
      </el-table-column>
      <el-table-column align="center" label="图片" prop="pic" v-slot="{row}">
        <el-image :src="row.pic?row.pic[0]:''" :preview-src-list="row.pic" style="height: 80px">
          <template slot="error">
            <span></span>
          </template>
        </el-image>
      </el-table-column>
      <el-table-column align="center" label="联系方式" prop="contactDetail"></el-table-column>
      <el-table-column align="center" label="更新时间" prop="crtTime"></el-table-column>
      <el-table-column align="center" label="是否采纳" prop="adoption" v-slot="{row}">
        <span v-if="adoptType[row.adoption]===undefined">您暂未查看</span>
        <span v-else :style="`color:${adoptType[row.adoption]==='未采纳'?color.danger:color.success}`">{{adoptType[row.adoption]}}</span>
      </el-table-column>
    ...
    <el-dialog modal-append-to-body append-to-body title="意见反馈" :visible.sync="isShowFeedbackDetail"
      ...
      <el-form :model="dataForm" label-width="120px" label-position="right" style="width: 90%">
        ...
        <el-form-item label="图片：">
          <el-image :src="dataForm.pic?dataForm.pic[0]:''" :preview-src-list="dataForm.pic" style="height: 80px">
            <template slot="error">
              <span></span>
            </template>
          </el-image>
        ...
        <el-form-item label="是否采纳：">
          <el-radio-group v-model="dataForm.adoption">
            <el-radio :label="Number(value)" v-for="(key,value) in adoptType">{{key}}</el-radio>
          </el-radio-group>
        </el-form-item>
...
<script>
  import { selectPage as pages, updObj,deleteBatch } from '@/api/erp/operation/feedback'
  import { adoptType } from './common/common'
  ...
  export default {
    ...
    methods: {
      ...
      getList() {
        ...
        pages(this.listQuery).then(response => {
          ...
          for (let i = 0; i <this.list.length ; i++) {
            this.list[i].pic=this.list[i]?.pic?.split(',')
          }
      ...
      handleSubmit() {
        this.isLoading = true
        this.dataForm.pic=this.dataForm.pic.toString()
