  ...
  <div class="task-div">
    <el-container>
      <el-header>
        ...
        <el-input clearable class="filter-item" style="width: 200px;" placeholder="执行人" v-model="listQuery.executorName"
                  @keyup.enter.native="handleFilter">
        </el-input>
        <el-date-picker clearable style="width: 200px;" v-model="listQuery.beginDate" type="date"
                        placeholder="制单开始时间" value-format="yyyy-MM-dd" format="yyyy-MM-dd"></el-date-picker>
        <el-date-picker clearable style="width: 200px;" v-model="listQuery.endDate" type="date"
                        placeholder="制单结束时间" value-format="yyyy-MM-dd" format="yyyy-MM-dd"></el-date-picker>
      ...
      <el-container>
        <!--列表-->
        <el-aside width="40%">
          <div class="task-item" v-if="list && list.length>0" v-for="item in list" :key="item.id"
            ...
            <el-card>
              ...
              <div style="padding: 15px; font-size: 16px">
                ...
                <div class="card-bottom">
                  <span>执行人：{{item.executorName}}</span><br>
                  <span>{{item.crtTime}}</span>
                  <div class="card-function">
                    <el-button type="text" @click.stop="handleEditState(item,1)" v-if="item.state==0"><i
                      class="iconfont icon-gou"></i>完成
                    </el-button>
                    <el-button type="text" @click.stop="handleEditState(item,0)" v-if="item.state==1"><i
                      class="iconfont icon-shuaxin1"></i>重启
                    </el-button>
                    <el-button type="text" @click.stop="openTaskRecordDialog(item.id)" v-if="item.state==1"><i
                      class="iconfont icon--pingfen"></i>评分
                    </el-button>
                    <!--<el-button type="text" @click.stop="handleReplyShow(item.id)"><i
                      class="iconfont icon-queren2"></i>确认
                    </el-button>-->
                    ...
                    <el-dropdown trigger="click">
                      ...
                      <el-dropdown-menu slot="dropdown" class="more-dropdown">
                        ...
                        <el-dropdown-item>
                          <el-button type="text" @click.stop="openTaskForwardDialog(item.id)"><i
