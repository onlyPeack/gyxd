<template>
  <el-dialog title="日志评分" :visible.sync="dialogVisible" width="30%" @close="handleDailyRecordClose"
             class="dailyRecordDialog"
             :close-on-click-modal="false" :before-close="handleDailyRecordClose">
    ...
    <div slot="footer" class="dialog-footer">
      <el-button @click="handleDailyRecordClose">取 消</el-button>
...
<script>
  import {getObj, putObj} from '@/api/erp/oa/daily';
  ...
  export default {
    ...
    methods: {
      ...
      handleDailyRecordClose() {
        this.$emit('closeDailyRecordDialog', false);
      },
      handleSubmit() {
        ...
        putObj(this.dataForm.id, this.dataForm).then(response => {
          if (response.code == '2000') {
            ...
            this.$emit('dailyRecordSubmitSuccess', this.dataForm.id);
          } else {
            this.$notify({
              title: '失败',
