<template>
  <div class="app-container calendar-list-container sales-detail-list-container full-purchase bill-container">
    ...
    <el-table size="small" :data="list" v-loading="listLoading" element-loading-text="正在查询中。。。" border fit
              v-if="!isChart" ref="analysisTable"
      ...
      <el-table-column align="right" label="考核金额" prop="checkAmount" sortable></el-table-column>
      <el-table-column align="right" label="仓库费用" prop="warehouseAmount" sortable></el-table-column>
      <el-table-column align="right" label="毛利" prop="profit" sortable></el-table-column>
      <el-table-column align="right" label="毛利率" prop="profitRate" sortable v-slot="{row}">
        <span>{{toPercent(division(row.profitRate,100))}}</span>
      </el-table-column>
      <el-table-column align="right" label="毛利(除去物流费)" prop="jlr" sortable v-slot="{row}">
        <span>{{accAdd(row.profit,-row.warehouseAmount)}}</span>
      </el-table-column>
      <el-table-column align="right" label="毛利率(除去物流费)" sortable v-slot="{row}" prop="jlrl">
        <span>{{toPercent(division(accAdd(row.profit,-row.warehouseAmount),Number(row.outAmount)).toFixed(4))}}</span>
      </el-table-column>
    </el-table>
    ...
    <el-dialog title="请选择品牌" :visible.sync="showBrandSelector" append-to-body width="60%">
      <!--    品牌选择弹窗-->
      <brand-selector v-if="showBrandSelector" :dialogVisible="showBrandSelector" :isSingle="false" :brandList="brandList"
...
<script>
  import {findAnalysisList as page} from '@/api/erp/wms/bill/stockOutAnalysis';
    ...
    methods: {
      ...
      getSummaries(param) {
        ...
        columns.forEach((column, index) => {
          ...
          if (column.label === '出库金额' || column.label === '考核金额' || column.label === '调拨金额' || column.label === '仓库费用' || column.property === 'profit') {
      ...
      excelTable() {
        import('@/vendor/Export2Excel').then(excel => {
          ...
          excel.export_json_to_excel2(tHeader, list, filterVal, '销售毛利统计表');
        })
      },
