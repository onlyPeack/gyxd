<template>
  <div class="app-container calendar-list-container sales-detail-list-container full-purchase bill-container">
    ...
    <el-table size="small" :data="list" v-loading="listLoading" element-loading-text="正在查询中。。。" border fit ref="analysisTable"
      ...
      <el-table-column align="right" label="订单状态" prop="analysisType" v-slot="{row}">
        <span>{{orderTypeMap[row.analysisType]}}</span>
      </el-table-column>
      <el-table-column align="right" label="总订单数" prop="count" sortable v-slot="{row}">
        <span>{{Number(row.count).toFixed(0)}}</span>
      </el-table-column>
      <el-table-column align="right" label="关闭订单数" prop="closeCount" sortable v-slot="{row}">
        <span>{{Number(row.closeCount).toFixed(0)}}</span>
      </el-table-column>
      <el-table-column align="right" label="订单关闭率" prop="salesmanNo" sortable v-slot="{row}">
        <span>{{toPercent((division(row.closeCount,row.count)||0).toFixed(4))}}</span>
        <!--        <span>{{(division(accAdd(row.count,-row.closeCount),row.count)||0)}}</span>-->
      </el-table-column>
      <el-table-column align="right" label="订单转化率" prop="orderProbability" sortable v-slot="{row}">
        <span>{{toPercent((division(row.count,orderSum)||0).toFixed(4))}}</span>
        <!--        <span>{{(division(accAdd(row.count,-row.closeCount),row.count)||0)}}</span>-->
      </el-table-column>
...
<script>
  import { page } from '@/api/erp/analysis/sales/orderConversion'
      ...
      getList() {
        ...
        page(this.listQuery).then(response => {
          ...
          for (let i in response) {
            this.chartData.push({ value: response[i].totalAmount, name: this.orderTypeMap[response[i].analysisType] })

          }
          this.list = response.reverse()
          this.orderSum=this.list[0].count
