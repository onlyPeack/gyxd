<template>
  <div class="app-container calendar-list-container sales-detail-list-container full-purchase bill-container">
    ...
    <el-table size="small" :data="list" v-loading="listLoading" element-loading-text="正在查询中。。。" border fit ref="analysisTable"
      ...
      <el-table-column align="center" label="业务员工号" prop="userNo"></el-table-column>
      <el-table-column align="center" label="业务员" prop="userName"></el-table-column>
      ...
      <el-table-column align="right" label="销售开票金额" prop="invoiceAmount" sortable></el-table-column>
      <el-table-column align="right" label="销售退票金额" prop="invoiceBackAmount" sortable></el-table-column>
      <el-table-column align="right" label="应收账款余额" prop="__ReceivableAmount" sortable></el-table-column>
...
<script>
  import {page} from '@/api/erp/analysis/sales/customerReceivable';
    ...
    methods: {
      ...
      getList() {
        ...
        page(this.listQuery).then(response => {
          for (let i = 0; i <response.length ; i++) {
            //应收账款余额=客户应收账款余额=销售开票-销售退票-销售收款+销售退款
            response[i].__ReceivableAmount = Number(Number(response[i].invoiceAmount)-Number(response[i].invoiceBackAmount)-Number(response[i].saleReceiveAmount)+Number(response[i].saleRefundMoneyAmount)).toFixed(4)
          }
          this.list = response;
      ...
      getSummaries(param) {
        ...
        columns.forEach((column, index) => {
          ...
          if (!values.every(value => isNaN(value))) {
            if (column.label === '销售出库金额' || column.label === '直发出库金额' ||column.label === '销售退库金额'||column.label==='销售退款金额'||column.label==='销售收款金额'||column.label==='销售开票金额'||column.label==='销售退票金额'||column.label==='应收账款余额') {
      ...
      excelTable(){
        import('@/vendor/Export2Excel').then(excel => {
          ...
          excel.export_json_to_excel2(tHeader, list, filterVal, '应收分析表');
        })
      },
    },
    watch: {
