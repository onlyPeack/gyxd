<template>
  <div class="app-container calendar-list-container sales-detail-list-container full-purchase bill-container">
    ...
    <div class="filter-container sales-detail-list-filter-container">
      <div>
        <!--        <el-select v-model="listQuery.analysisType" placeholder="分析类型" clearable style="width: 200px;" disabled-->
        ...
        <!--        </el-input>-->
        <div style="display: inline-block" @click="customerVisible=true">
    ...
    <el-table size="small" :data="list" v-loading="listLoading" element-loading-text="正在查询中。。。" border fit
      ...
      <el-table-column align="right" label="省份" prop="analysisType"></el-table-column>
      <el-table-column align="right" label="客户数量" prop="count" sortable></el-table-column>
      <el-table-column align="right" label="出库金额" prop="outAmount" sortable></el-table-column>
      <el-table-column align="right" label="出库毛利" prop="profit" sortable></el-table-column>
      <el-table-column align="right" label="出库毛利率" prop="profitRate" sortable></el-table-column>
      <el-table-column align="right" label="月增长客户数" prop="monthCustomerIncreaseCount" sortable v-slot="{row}">
        <span>{{Number(row.monthCustomerIncreaseCount).toFixed(0)}}</span>
      </el-table-column>
      <el-table-column align="right" label="月增长销售额" prop="monthSaleIncreaseAmount" sortable></el-table-column>
    ...
    <div style="height: 680px" v-if="isChart">
...
<script>
  ...
  import {findAreaAnalysisList as page} from '@/api/erp/wms/bill/stockOutAnalysis'
  import {getFloat} from '@/utils/util'
  import baiduMapStyle from '@/assets/baidu/baiduMapStyle.json'
  export default {
    name: 'salesArea',
    components: {
    ...
    methods: {
      ...
      getList() {
        ...
        page(this.listQuery).then(response => {
          ...
          for (let i = 0; i < data.length; i++) {
            if (data[i].analysisType.length > 2) {
              data[i].analysisType = data[i].analysisType.substring(0, 2)
            }
            this.chartData.push({
              value2: data[i].count,
              name: data[i].analysisType,
              value: this.division(data[i].outAmount, 10000).toFixed(2)
            })
          }
          this.list = response.data
          this.total = response.data.length
          this.listLoading = false
          console.log(this.chartData, 'char')
        }).catch(() => {
          this.list = []
          this.total = 0
          this.listLoading = false
      ...
      getSummaries(param) {
        ...
        columns.forEach((column, index) => {
          ...
          if (column.property == 'outAmount' || column.property == 'checkAmount' || column.property == 'profit' || column.property === 'monthSaleIncreaseAmount') {
          ...
          } else if (column.property === 'monthCustomerIncreaseCount') {
            let sum = 0
            for (let i = 0; i < data.length; i++) {
              sum += data[i].monthCustomerIncreaseCount * 1
            }
            sums[index] = sum.toFixed(0)
          }
          // 出库金额合计
          ...
          myChart.setOption({
            tooltip: {
              formatter: function(params, ticket, callback) {
                ...
                return params.name + '<br />' + '客户数量' + '：' + params.data.value2 + '<br/>' + '销售额：' + (params.value || 0) + '万'
              }
            },
            visualMap: {
              min: 0,
      ...
      closeDepartDialog(depart) {
        ...
        let departNos = []
        console.log(depart)
        this.listQuery.departNo = depart.checkedKeys.toString()
      ...
      excelTable() {
        import('@/vendor/Export2Excel').then(excel => {
          ...
          for (let i = 0; i < list.length; i++) {
            list[i].index = i + 1
            list[i].monthCustomerIncreaseCount = Number(list[i].monthCustomerIncreaseCount).toFixed(0)
          ...
          excel.export_json_to_excel2(tHeader, list, filterVal, '销售区域分析表');
        })
      },
