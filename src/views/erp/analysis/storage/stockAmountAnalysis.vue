...
<!--    </div>-->
    ...
    <el-table size="small" :data="list" v-loading="listLoading" element-loading-text="正在查询中。。。" border fit ref="analysisTable"
      ...
      <el-table-column align="right" label="库存进价金额" prop="stockCostAmount" sortable></el-table-column>
      <el-table-column align="right" label="库存考核金额" prop="stockCheckAmount" sortable></el-table-column>
      <el-table-column align="right" label="三个月以内库存" prop="stockThreeMonthAmount" sortable></el-table-column>
      <el-table-column align="right" label="超三个月库存" prop="stockOverThreeMonthAmount" sortable></el-table-column>
      <el-table-column align="right" label="超六个月库存" prop="stockOverSixMonthAmount" sortable></el-table-column>
    </el-table>
...
<script>
  import {page} from '@/api/erp/analysis/storage/stockAmountAnalysis';
  import {getFloat} from '@/utils/util';

  export default {
    name: 'stockAmountAnalysis',
    components: {
    ...
    methods: {
      ...
      excelTable() {
        import('@/vendor/Export2Excel').then(excel => {
          ...
          excel.export_json_to_excel2(tHeader, list, filterVal, '库存余额综合统计');
        })
      },
