<template>
  <div class="full-purchase bill-container">
      ...
      <div class="query-container" v-show="queryVisible">
        <div class="block">
          ...
          <el-date-picker
            ...
            @change="handleContractDateSelect" style="margin-right: 2px;"
          ...
          <el-date-picker
            ...
            :default-time="['00:00:00', '23:59:59']"
            @change="handleEffectiveDateDateSelect" style="margin-right: 2px;"
            :picker-options="pickerOptions">
          </el-date-picker>
          <el-date-picker
            ...
            :default-time="['00:00:00', '23:59:59']"
            @change="handleDisabledDateSelect" style="margin-right: 2px;"
            :picker-options="pickerOptions">
          </el-date-picker>
        </div>
      </div>
    ...
    <el-table :data="list" border style="width: 100%" v-loading="listLoading" ref="dataTable"
      ...
      <el-table-column prop="mainDTO.billNo" label="合同编号" width="160" sortable>
      ...
      <el-table-column prop="crtUserNo" label="业务员工号" width="90">
      </el-table-column>
      <el-table-column prop="mainDTO.salerName" label="业务员姓名" width="90">
      </el-table-column>
      <el-table-column prop="departName" label="业务员部门" width="120">
      </el-table-column>
      ...
      <el-table-column prop="mainDTO.effectiveDate" label="生效日期" width="108" sortable>
      </el-table-column>
      <el-table-column prop="mainDTO.disabledDate" label="到期日期" width="108" sortable>
      </el-table-column>
    ...
    <el-table :data="list" border style="width: 100%" v-loading="listLoading" ref="dataTable"
      ...
      <el-table-column prop="billNo" label="合同编号" width="160" sortable>
        <template slot-scope="scope">
      ...
      <el-table-column prop="salerName" label="业务员姓名" width="90">
      </el-table-column>
      <el-table-column prop="departName" label="业务员部门" width="120">
      </el-table-column>
      <el-table-column prop="contractAmount" label="合同金额" align="right" width="108" class-name="col-required">
      </el-table-column>
      <el-table-column prop="customerOrderQty" label="商品数量" width="105">
      </el-table-column>
      <!--<el-table-column prop="customerOrderStatus" label="客户订货状态" width="100">-->
        <!--<template slot-scope="scope">-->
          <!--<div v-for="item in orderGoodsStatusOptions">-->
            <!--<span v-if="item.value==scope.row.customerOrderStatus">{{item.label}}</span>-->
          <!--</div>-->
        <!--</template>-->
      <!--</el-table-column>-->
      <!--<el-table-column prop="stockOutStatus" label="出库状态" width="100">-->
        <!--<template slot-scope="scope">-->
          <!--<div v-for="item in storageOutStatusOptions">-->
            <!--<span v-if="item.value==scope.row.stockOutStatus">{{item.label}}</span>-->
          <!--</div>-->
        <!--</template>-->
      <!--</el-table-column>-->
      <!--<el-table-column prop="returnStatus" label="回款状态" width="100">-->
        <!--<template slot-scope="scope">-->
          <!--<div v-for="item in returnMoneyStatusOptions">-->
            <!--<span v-if="item.value==scope.row.returnStatus">{{item.label}}</span>-->
          <!--</div>-->
        <!--</template>-->
      <!--</el-table-column>-->
      <!--<el-table-column prop="invoiceStatus" label="开票状态" width="100">-->
        ...
        <!--</template>-->
      <!--</el-table-column>-->
      <el-table-column prop="contractDate" label="签约日期" width="108" sortable>
      </el-table-column>
      <el-table-column prop="effectiveDate" label="生效日期" width="108" sortable>
      </el-table-column>
      <el-table-column prop="disabledDate" label="到期日期" width="108" sortable>
      </el-table-column>
      <el-table-column prop="crtTime" label="制单时间" width="140" sortable>
      </el-table-column>
    ...
    <!-- 新增销售合同弹窗 -->
    <sales-contract-form :dialogFormVisible="dialogFormVisible" v-if="dialogFormVisible" :id="id"
                         @submitSuccess="submitSuccess" @handleClose="handleClose">
    </sales-contract-form>

    <!-- 销售合同详情弹窗 -->
    <sales-contract-view :dialogFormVisible="dialogDetailVisible" v-if="dialogDetailVisible" :id="id"
...
<script>
  import {selectPage, selectPageByItem, batchDelBill} from '@/api/erp/sales/bill/salesContract';
  import {getFloat} from '@/utils/util';

  export default {
    name: "salesContractList",
    components: {
      'sales-contract-form': () => import('./form'),
      'sales-contract-view': () => import('./view')
    },
    data() {
      return {
    ...
    methods: {
      handleDownload() {
        ...
        import('@/vendor/Export2Excel').then(excel => {
          ...
          if (this.choose == false) {
            tHeader = ['签约日期', '生效日期', '到期日期', '合同编号', '合同名称', '客户名称', '业务员', '合同金额', '单据状态', '商品数量', '客户订货状态', '出库状态', '回款状态', '开票状态', '备注', '打印次数'];
            filterVal = ['contractDate', 'effectiveDate', 'disabledDate', 'billNo', 'contractName', 'customerName', 'salerName', 'contractAmount', 'billStatus', 'customerOrderQty', 'customerOrderStatus', 'stockOutStatus', 'returnStatus', 'invoiceStatus', 'note', 'printCount'];
          } else {
            for (var i in list) {
              Object.assign(list[i], list[i].mainDTO);
              delete list[i].mainDTO;
            }
            tHeader = ['签约日期', '生效日期', '到期日期', '合同编号', '合同名称', '客户名称', '业务员', '合同金额', '单据状态', '客户订货状态', '出库状态', '回款状态', '开票状态', '商品编码', '商品名称', '订货号', '规格型号', '数量', '单价', '金额', '税额', '含税单价', '价税合计', '备注', '打印次数'];
            filterVal = ['contractDate', 'effectiveDate', 'disabledDate', 'billNo', 'contractName', 'customerName', 'salerName', 'contractAmount', 'billStatus', 'customerOrderStatus', 'stockOutStatus', 'returnStatus', 'invoiceStatus', 'skuNo', 'skuName', 'itemNo', 'specifications', 'qty', 'price', 'amount', 'tax', 'taxPrice', 'taxAmount', 'note', 'printCount'];
          }
          excel.export_json_to_excel2(tHeader, list, filterVal, '销售合同');
      ...
      handleReset() {
        this.listQuery.billNo = undefined,
          this.listQuery.contractName = undefined,
