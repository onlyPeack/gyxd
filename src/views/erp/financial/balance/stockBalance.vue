<template>
  ...
  <div class="app-container calendar-list-container bill-container">
    ...
    <el-table size="small" :data="list" v-loading="listLoading" ref="dataTable"
      ...
      <el-table-column align="right" label=" 库存账户资金" prop="stockAmount" class-name="col-required">
        <template slot-scope="scope">
          {{Number(scope.row.stockAmount).toFixed(4)}}
        </template>
      </el-table-column>
      <el-table-column align="right" label="实际库存资金" prop="stockSumAmount" class-name="col-required">
        <template slot-scope="scope">
          {{Number(scope.row.stockSumAmount).toFixed(4)}}
        </template>
      </el-table-column>
    </el-table>
...
<script>
  import {stockPage} from '@/api/erp/financial/balance';
  ...
  export default {
    ...
    methods: {
      getList() {
        ...
        stockPage().then(response => {
          this.differenceMount = (response.stockAmount- response.stockSumAmount).toFixed(4)
          this.list = [response];
