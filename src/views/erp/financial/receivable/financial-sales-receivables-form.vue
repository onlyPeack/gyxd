<template>
  <el-dialog :visible.sync="dialogFormVisible" width="70%" @close="handleClose" class="billFormDialog"
      ...
      <span slot="title" class="con-title">
        ...
        <span v-if="!dataForm.id">新增销售收款</span>
        <span v-if="dataForm.id">编辑销售收款</span>
    ...
    <div style="display: flex;flex-direction: row;justify-content: flex-start;">
      <el-upload class="upload"  action="/api/financial/sales-collections/import" :before-upload="beforeUpload"
        ...
        <el-button type="text" icon="el-icon-download">
          <a href="/api/financial/sales-collections/downTemplate">下载导入模板</a>
        </el-button>
      </el-upload>
    </div>
    <el-table :data="items" border highlight-current-row width="100%" :height="clientHeight"
      ...
      <!--                    @change="handleEdit('thisReceivableAmount',$event,scope.$index,scope.row)"></el-input>-->
      ...
      <el-table-column prop="amountFormer" label="到款金额" align="right" class-name="col-required">
        <template slot-scope="scope">
          <el-input v-model="scope.row.amountFormer" placeholder="请输入到款金额(正数为收款,负数为退款)" class="right-el-input"
                    @change="handleEdit('amountFormer',$event,scope.$index,scope.row)"></el-input>
                    <span>{{dataForm.amountFormer}}</span>
        </template>
      </el-table-column>
      <el-table-column prop="unit" label="到款日期" width="150">
        <template slot-scope="scope">
          <el-date-picker v-model="scope.row.receiveDate" type="date" value-format="yyyy-MM-dd" :picker-options="pickerOptions"
                          placeholder="请选择日期">
          </el-date-picker>
...
<script>
  ...
  import { addBill, getBill, tempAddBill } from '@/api/erp/financial/financialSalesReceivables'
  import { getObj } from '@/api/admin/user/index'
  import { pageAll } from '@/api/erp/financial/financialBankAccount'
  import {getToken} from '@/utils/auth'
  export default {
    name: 'financial-sales-receivables-form',
    components: {
    ...
    data() {
      const validateAmountFormer = (rule, value, callback) => {
        if (!value || value == '' || value == '0.00' || value == 'NaN') {
          callback(new Error('请输入回款金额(不能为0)'))
        } else {
          callback()
        }
      }
    ...
    methods: {
        ...
        if (response.code == '2000') {
          ...
          if(response.data.length>0){
            ...
            for (var index in this.selectedProducts) {
              ...
              var itemObj = {};
              itemObj.amountFormer = item.amountFormer.toFixed(4);
      ...
      closeCustomerDialog(list) {
        ...
        if (list.length > 0) {
          ...
          // this.salerArr = list[0].customerManagerArr
      ...
      checkItemFuc() {
        for (var i in this.items) {
          ...
          if (!this.items[i].amountFormer || this.items[i].amountFormer == '') {
            this.$message.error('第' + (i * 1 + 1) + '行的到款金额未填写，请填写了再提交')
            return false
          }
