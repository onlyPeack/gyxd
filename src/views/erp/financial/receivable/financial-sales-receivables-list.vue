<template>
  <div class="full-purchase bill-container">
    ...
    <div class="filter-container">
      ...
      <div class="query-container">
        ...
        <div>
          ...
          <el-button class="filter-item" type="primary" :loading="isSync" v-if="$store.getters.groupNames.indexOf('财务') > -1 || $store.getters.groupNames.indexOf('总经理') > -1" @click="handleSyncRemitInfo">同步工行打款信息</el-button>
        </div>
      </div>
    </div>
    <div style="margin-top: 1px;">
      <el-button type="text" icon="el-icon-plus" @click="handleCreate" id="add_bill" v-if="listQuery.roleType == 0">新建
      </el-button>
      <el-button type="text" icon="el-icon-edit" @click="handleUpdate" id="edit_bill" v-if="listQuery.roleType == 0">
        编辑
      </el-button>
      <el-button type="text" icon="el-icon-delete" @click="handleDelete" id="delete_bill"
                 v-if="listQuery.roleType == 0">删除
      </el-button>

      <el-button type="text" id="export_bill" :loading="downloadLoading" @click="handleDownload"
                 v-if="listQuery.roleType == 0 && choose">
    ...
    <el-table v-if="!choose" :data="list" border style="width: 100%" v-loading="listLoading" ref="dataTable"
      ...
      <el-table-column prop="receiveAccount" label="收款账户" align="center">
      </el-table-column>
      <el-table-column prop="paymentsName" label="结算方式" align="center">
      </el-table-column>
      <el-table-column prop="note" label="备注">
      </el-table-column>
      <el-table-column prop="crtTime" label="单据日期" align="center" sortable>
      </el-table-column>
      <!--<el-table-column prop="currencyName" label="币别" width="150">-->
      <!--</el-table-column>-->
      <!--<el-table-column prop="paymentFormName" label="回款形式" width="150">-->
      <!--</el-table-column>-->
      <!--<el-table-column prop="invoiceStatusName" label="开票状态" width="150">-->
      <!--</el-table-column>-->
    ...
    <!-- 新增实际回款弹窗 -->
    <financial-sales-receivables-form :dialogFormVisible="dialogFormVisible" v-if="dialogFormVisible" :id="id"
                                      @submitSuccess="submitSuccess" @handleClose="handleClose"
                                      :paymentsMeansOptions="paymentsMeansOptions">
    </financial-sales-receivables-form>

    <!-- 实际回款详情弹窗 -->
    <financial-sales-receivables-view :dialogFormVisible="dialogDetailVisible" v-if="dialogDetailVisible" :id="id"
                                      @submitDetailSuccess="submitDetailSuccess" @handleClose="handleDetailClose"
                                      :paymentsMeansOptions="paymentsMeansOptions">
    </financial-sales-receivables-view>
...
<script>
  ...
  export default {
    name: 'financial-sales-receivables-list',
    components: {
      'financial-sales-receivables-form': () => import('./financial-sales-receivables-form'),
      'financial-sales-receivables-view': () => import('./financial-sales-receivables-view')
    },
    data() {
      return {
        fileList: [],
    ...
    methods: {
      handleSyncRemitInfo() {
        this.isSync = true;
        manualGenerateBatchExternalSCBill().then(() => {
      ...
      handleDownload() {
        ...
        import('@/vendor/Export2Excel').then(excel => {
          ...
          excel.export_json_to_excel2(tHeader, list, filterVal, '销售收款')
      ...
      handleExceed(files, fileList) {
        this.$message.warning(`当前限制选择 5个文件，本次选择了 ${files.length} 个文件，共选择了 ${files.length} 个文件`)
      },
