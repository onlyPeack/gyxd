<template>
  <div class="app-container calendar-list-container">
    ...
    <el-table size="small" :data="list" v-loading="listLoading" element-loading-text="正在查询中。。。" border fit
      ...
      <el-table-column align="center" label="简称" prop="abbrName">
      </el-table-column>
      <el-table-column align="center" label="账户编号" prop="accountCode">
      </el-table-column>
      <!--<el-table-column align="center" label="结算方式" prop="paymentsMeans">-->
        <!--<template slot-scope="scope">-->
          <!--<span v-for="item in paymentsMeansList">-->
            <!--<span v-if="scope.row.paymentsMeans ==item.value">{{item.label}}</span>-->
          <!--</span>-->
        <!--</template>-->
      <!--</el-table-column>-->
      <!--<el-table-column align="center" label="是否辅助核算" prop="isAuxiliaryAccount">-->
        <!--<template slot-scope="scope">-->
          <!--<span v-if="scope.row.isAuxiliaryAccount ===1">是</span>-->
          <!--<span v-else>否</span>-->
        <!--</template>-->
      <!--</el-table-column>-->
      <!--<el-table-column align="center" label="科目" prop="accountName">-->
      <!--</el-table-column>-->
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible">
      <el-form :model="dataForm" :rules="rules" ref="dataForm" label-width="100px">
        ...
        <el-form-item label="简称" prop="abbrName">
          <el-input v-model="dataForm.abbrName" clearable></el-input>
        </el-form-item>
        <el-form-item label="账户编号" prop="accountCode">
          <el-input v-model="dataForm.accountCode" clearable :disabled="dialogStatus=='update'"></el-input>
        </el-form-item>
        <el-form-item label="排序" prop="sort">
          <el-input v-model="dataForm.sort" clearable></el-input>
        </el-form-item>
        <!--<el-form-item label="结算方式" prop="paymentsMeans">-->
          <!--<el-select v-model="dataForm.paymentsMeans" style="width: 200px;" class="filter-item" filterable>-->
            <!--<el-option v-for="item in paymentsMeansList" :key="item.value" :label="item.label"-->
                       <!--:value="item.value"></el-option>-->
          <!--</el-select>-->
        <!--</el-form-item>-->
        <!--<el-form-item label="是否辅助核算" prop="isAuxiliaryAccount">-->
          <!--<el-checkbox v-model="dataForm.isAuxiliaryAccount" true-label=1 false-label=0></el-checkbox>-->
        <!--</el-form-item>-->
        <!--<el-form-item label="科目" prop="accountName">-->
          <!--<el-input v-model="dataForm.accountName" clearable></el-input>-->
        <!--</el-form-item>-->
        <!--<el-form-item label="科目编码" prop="accountNumber">-->
          <!--<el-input v-model="dataForm.accountNumber" clearable></el-input>-->
        <!--</el-form-item>-->
...
<script>
  import {page, addObj, getObj, delObj, putObj, updateStatus} from '@/api/erp/financial/financialBankAccount';
  import {page as selectPayMeans} from '@/api/erp/financial/financialPaymentsMeans';
  ...
  export default {
    ...
    data() {
      return {
        ...
        rules: {
          ...
          accountNo: [{required: true, message: '账户不能为空', trigger: 'blur'}],
          paymentsMeans: [{required: true, message: '结算方式不能为空', trigger: 'blur'}],
        }
      }
    },
    created() {
    ...
    methods: {
      init() {
        selectPayMeans().then(response => {
          this.paymentsMeansList = response.data.records;
        })
      },
      getList() {
