<template>
  <div class="app-container calendar-list-container sales-summary-list-container bill-container">
    <el-tabs v-model="selectType">
      <el-tab-pane label="公司资金账户" name="1">
        <!-- 查询和其他操作 -->
        <div class="filter-container corporate-account-filter-container">
          <div>
        ...
        <el-table size="small" :data="list" v-loading="listLoading" element-loading-text="正在查询中。。。" border fit
                  ...
                  :summary-method="getSummaries" :default-sort="{prop: 'orderNo', order: 'descending'}">
          ...
          <el-table-column align="right" label="账户总金额" prop="amount" sortable class-name="col-required">
            <template slot-scope="scope">{{scope.row.amount.toFixed(4)}}</template>
          </el-table-column>
          <!--<el-table-column align="right" label="可用金额" prop="availableAmount" sortable class-name="col-required">-->
          <!--</el-table-column>-->
      ...
      <el-tab-pane label="公司账户交易记录" name="2">
        <!-- 查询和其他操作 -->
        <div class="filter-container corporate-account-filter-container">
          <div>
        ...
        <div class="pagination-container" style="margin-top: 10px;">
          <el-pagination background @size-change="handleFlowSizeChange" @current-change="handleFlowCurrentChange"
                         ...
                         :page-sizes="[10,20,30,50,500,1000]" :page-size="flowListQuery.limit"
...
<script>
  ...
  export default {
    ...
    methods: {
      handleDownload() {
        ...
        import('@/vendor/Export2Excel').then(excel => {
          ...
          if (this.selectType == '1') {
            tHeader = ['账户号', '账户名称', '账户总金额', '可用余额', '账户状态', '创建时间', '最后更新时间', '备注'];
            ...
            excel.export_json_to_excel2(tHeader, list, filterVal, '公司账户交易记录');
          }
      ...
      changeDivHeight() {
        if (document.getElementsByClassName('corporate-account-filter-container')[0]) {
          if (this.selectType == '1') {
            var height = document.getElementsByClassName('corporate-account-filter-container')[0].clientHeight;
          } else {
            var height = document.getElementsByClassName('corporate-account-filter-container')[1].clientHeight;
          }
        } else {
          height = 48;
        }
      ...
      getSummaries(param) {
        ...
        columns.forEach((column, index) => {
          ...
          if (!values.every(value => isNaN(value))) {
            if (column.property == 'amount' || column.property == 'afterAmount') {
