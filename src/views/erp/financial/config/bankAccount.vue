<template>
  <div class="app-container calendar-list-container sales-summary-list-container bill-container">
    <el-tabs v-model="selectType">
      <el-tab-pane label="银行资金账户" name="1">
        <!-- 查询和其他操作 -->
        ...
        <el-table size="small" :data="list" v-loading="listLoading" element-loading-text="正在查询中。。。" border fit
          ...
          <el-table-column align="right" label="账户余额" prop="amount" class-name="col-required">
            <template slot-scope="scope">{{scope.row.amount.toFixed(4)}}</template>
          </el-table-column>
          <el-table-column align="right" label="可用余额" prop="availableAmount" class-name="col-required">
      ...
      <el-tab-pane label="银行交易记录" name="2">
        <!-- 查询和其他操作 -->
        <div class="filter-container bank-account-filter-container">
          <div>
      ...
      changeDivHeight() {
        if (document.getElementsByClassName('bank-account-filter-container')[0]) {
          if (this.selectType == '1') {
            ...
            var height = document.getElementsByClassName('bank-account-filter-container')[1].clientHeight;
          }
        } else {
          height = 48;
        }
