<template>
  <div class="app-container calendar-list-container sales-summary-list-container bill-container">
    <el-tabs v-model="selectType">
      <el-tab-pane label="库存资金账户" name="1">
        <!-- 查询和其他操作 -->
        <div class="filter-container stock-account-filter-container">
          <div>
        ...
        <el-table size="small" :data="list" v-loading="listLoading" element-loading-text="正在查询中。。。" border fit
          ...
          <el-table-column align="left" label="账户号" prop="accountCode">
          </el-table-column>
          <el-table-column align="left" label="账户名称" prop="accountName">
          </el-table-column>
          <el-table-column align="right" label="账户总金额" prop="amount" class-name="col-required">
            <template slot-scope="scope">{{scope.row.amount.toFixed(4)}}</template>
          </el-table-column>
          <!--<el-table-column align="right" label="可用金额" prop="availableAmount" class-name="col-required">-->
          <!--</el-table-column>-->
      ...
      <el-tab-pane label="库存交易记录" name="2">
        <!-- 查询和其他操作 -->
        <div class="filter-container stock-account-filter-container">
          <div>
      ...
      handleDownload() {
        ...
        import('@/vendor/Export2Excel').then(excel => {
          ...
          if (this.selectType == '1') {
            ...
            excel.export_json_to_excel2(tHeader, list, filterVal, '库存交易记录');
          }
          this.downloadLoading = false;
        })
      },
      changeDivHeight() {
        if (document.getElementsByClassName('stock-account-filter-container')[0]) {
          if (this.selectType == '1') {
            var height = document.getElementsByClassName('stock-account-filter-container')[0].clientHeight;
          } else {
            var height = document.getElementsByClassName('stock-account-filter-container')[1].clientHeight;
          }
        } else {
          height = 48;
        }
