<template>
  <div class="full-purchase bill-container">
      ...
      <div class="query-container">
        <div class="block">
          <el-date-picker
            ...
            @change="handleRepayMoneyDateSelect"
    ...
    <el-table :data="list" border style="width: 100%" v-loading="listLoading" ref="dataTable"
      ...
      <el-table-column prop="repayMoneyDate" label="还款日期" align="center">
      </el-table-column>
      <el-table-column prop="userName" label="还款人" width="150" align="center">
      </el-table-column>
      <el-table-column prop="payOrgName" label="还款部门" width="200" align="center">
      </el-table-column>
      <el-table-column prop="currencyId" label="币别" width="80" align="center">
        <template slot-scope="scope">
      ...
      <el-table-column prop="amount" label="还款金额" width="120" align="right" class-name="col-required">
        <template slot-scope="scope">
      ...
      <el-table-column prop="supplierName" label="关联供应商" width="120">
      </el-table-column>
      <el-table-column prop="voucherStatus" label="记账状态" width="100" align="center">
      </el-table-column>
    </el-table>
    ...
    <!-- 新增还款单弹窗 -->
    <repayment-money-form :dialogFormVisible="dialogFormVisible" v-if="dialogFormVisible" :id="id"
                          @submitSuccess="submitSuccess" @handleClose="handleClose"
                          :typeOptions="typeOptions">
    </repayment-money-form>

    <!-- 还款单详情弹窗 -->
    <repayment-money-view :dialogFormVisible="dialogDetailVisible" v-if="dialogDetailVisible" :id="id"
                          @submitDetailSuccess="submitDetailSuccess" @handleClose="handleDetailClose"
                          :typeOptions="typeOptions">
    </repayment-money-view>
...
<script>
  import {selectPage, batchDelBill} from '@/api/erp/financial/financialRepaymentMoney';
  import {getFloat} from '@/utils/util';

  export default {
    name: "repaymentMoneyList",
    components: {
      'repayment-money-form': () => import('./repaymentMoneyForm'),
      'repayment-money-view': () => import('./repaymentMoneyView')
    },
    data() {
      return {
        ...
        listQuery: {
          ...
          userName: undefined,
          startRepayMoneyDate: undefined,
          endRepayMoneyDate: undefined,
    ...
    methods: {
      handleDownload() {
        ...
        import('@/vendor/Export2Excel').then(excel => {
          ...
          tHeader = ['单据日期', '还款日期', '单据编号', '还款人', '还款部门','币别','还款金额','关联客户','关联供应商','单据状态','记账状态'];
          filterVal = ['billDate', 'repayMoneyDate', 'billNo', 'userName', 'payOrgName', 'currencyId','amount','customerName','supplierName','billStatus','voucherStatus'];
          excel.export_json_to_excel2(tHeader, list, filterVal, '还款单');
      ...
      handleRepayMoneyDateSelect(val) {
        this.listQuery.startRepayMoneyDate = val[0];
        this.listQuery.endRepayMoneyDate = val[1];
      },
    ...
    watch: {
      repayMoneyDate(value) {
        if (value == null) {
          this.listQuery.startRepayMoneyDate = undefined;
          this.listQuery.endRepayMoneyDate = undefined;
        }
      }
    }
  }
