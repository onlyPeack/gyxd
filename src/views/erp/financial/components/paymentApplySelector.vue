  ...
  <el-dialog title="选择关联单据" :visible.sync="dialogSelectorVisible" width="70%" @close="handleSelectorClose"
    ...
    <div class="full-purchase payment-apply-full-purchase">
      ...
      <el-table :data="list" border style="width: 100%" v-loading="listLoading" ref="dataTable"
        ...
        <el-table-column prop="billDate" label="单据日期" width="108">
        </el-table-column>
        ...
        <!--<el-table-column prop="paymentsMeans" label="结算方式" width="120">-->
        <!--<template slot-scope="scope">-->
        <!--<div v-for="item in paymentsMeansOptions">-->
        <!--<span v-if="scope.row.paymentsMeans==item.value">{{item.labelDefault}}</span>-->
        <!--</div>-->
        <!--</template>-->
        <!--</el-table-column>-->
        <el-table-column prop="currencyId" label="币别" width="80">
          <template slot-scope="scope">
          <span v-for="(item,index) in currencyOptions" :key="index">
        ...
        <el-table-column prop="paidAmount" label="已付款金额" width="120" align="right">
            <template slot-scope="scope">
            {{(scope.row.paidAmount).toFixed(4)}}
          </template>
        </el-table-column>
        <el-table-column label="剩余金额" width="120" align="right">
          <template slot-scope="scope">
            {{(scope.row.applyAmount - scope.row.paidAmount).toFixed(4)}}
          </template>
        </el-table-column>
