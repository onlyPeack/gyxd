<template>
  <el-dialog :visible.sync="dialogFormVisible" width="70%" @close="handleClose" class="billFormDialog"
      ...
      <span slot="title" class="con-title">
        ...
        <span v-if="!dataForm.id">新增采购合同</span>
        <span v-if="dataForm.id">编辑采购合同</span>
          ...
          <tr>
            <td>
              <el-form-item label="合同编号:" prop="billNo" class="el-form-flex">
                <el-input :value="dataForm.billNo ? dataForm.billNo : '合同编号由系统自动生成'" disabled></el-input>
              </el-form-item>
            </td>
            <td>
          ...
          <tr>
            <td>
              <el-form-item label="结算方式:" prop="payments" class="el-form-flex">
                <el-select v-model="dataForm.payments" style="width: 100%;">
    ...
    <el-table :data="items" border highlight-current-row width="100%" :height="clientHeight"
      ...
      <el-table-column prop="taxRate" label="税率" width="80" align="center">
        <template slot-scope="scope">
          ...
          <span>{{scope.row.taxRateOptions}}</span></template>
      </el-table-column>
      ...
      <el-table-column prop="deliveryDate" label="货期(天)" width="100" class-name="col-required">
        <template slot-scope="scope">
          <el-input size="mini" v-model="scope.row.deliveryDate"></el-input>
    ...
    <div slot="footer" class="dialog-footer">
      <div v-if="dataForm.billStatus && dataForm.billStatus==1">
        <el-button type="primary" style="float: left;">
          <router-link target="_blank" :to="{path:'/print/purchase-contract-print',query:{id:id}}">打 印
          </router-link>
        </el-button>
      </div>
...
<script>
  ...
  import {addBill, getBill, tempAddBill} from '@/api/erp/purchase/bill/purchaseContract';
    ...
    methods: {
      ...
      closeUserDialog(list) {
        ...
        if (list.length > 0) {
          ...
          this.dataForm.purchaserCode = list[0].code;
        }
      },
      ...
      submitSelectorSuccess(obj) {
        ...
        for (var index in obj.list) {
          ...
          if (index == 0) {
            ...
            _this.dataForm.purchaserCode = item.mainDTO.purchaserCode
        ...
        for (var i in this.items) {
          if (this.items[i].skuNo && this.items[i].skuNo != '') {
            ...
            if (!this.items[i].deliveryDate || this.items[i].deliveryDate == '') {
              this.$message.error('第' + (i * 1 + 1) + '行的货期必填项，请填写了再提交');
