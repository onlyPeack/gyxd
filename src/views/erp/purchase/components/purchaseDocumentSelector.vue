<template>
  <el-dialog title="选择关联单据" :visible.sync="dialogSelectorVisible" width="75%" @close="handleSelectorClose"
    ...
    <div class="full-purchase purchase-document-full-purchase">
      ...
      <div class="filter-container purchase-document-filter-container">
        <div>
          ...
          <el-input clearable class="filter-item" style="width: 200px;" placeholder="单据编号"
                    v-model="listQuery.billNo"
                    @keyup.enter.native="handleFilter" v-if="sourceObject.key=='CGDD'">
          </el-input>
          <el-input clearable class="filter-item" style="width: 200px;" placeholder="采购员"
                    v-model="listQuery.purchaserName"
                    @keyup.enter.native="handleFilter" v-if="sourceObject.key=='CGDD'">
      ...
      <el-table-column prop="paymentApplyReturnAmount" label="已申请金额" align="right" width="120" class-name="col-required">
        <template slot-scope="scope">
      ...
      <el-table :data="list" border style="width: 100%" v-loading="listLoading" ref="dataTable"
                ...
                highlight-current-row element-loading-text="正在查询中。。。" :height="450"
                show-summary :summary-method="getSummaries" class="result-data-table"
      ...
      <el-table-column prop="mainDTO.billNo" align="center" label="单据编号" min-width="180" sortable>
        <template slot-scope="scope">
          <div class="colum-billno" v-if="scope.row.mainDTO">
            {{scope.row.mainDTO.billNo}}
          </div>
        </template>
      </el-table-column>
...
<script>
  ...
  export default {
    ...
    props: ['dialogSelectorVisible', 'sourceBillNo','supplierCode'],
    data() {
      return {
        clientHeight: 300,
        list: [],
        ...
        listQuery: {
          ...
          selectType: 4 //单据查询时使用，0代表查所有状态，1代表只查正常状态（关联单据时）
        },
        selectedRows: [],
        queryVisible: false,
        billOptions: [{
    ...
    methods: {
      ...
      changeDivHeight() {
        if (document.getElementsByClassName('purchase-document-filter-container')[0]) {
          var height = document.getElementsByClassName('purchase-document-filter-container')[0].clientHeight;
        } else {
          height = 38;
        }
      ...
      getListByCGDD() {
        this.listLoading = true;
        selectPageCGDD(this.listQuery).then(response => {
          this.list = response.data.records;
          this.total = response.data.total;
          this.listLoading = false;
        }).catch(() => {
      ...
      getListByCGRKD() {
        this.listLoading = true;
        selectPageCGRKD(this.listQuery).then(response => {
          this.list = response.data.records;
          this.total = response.data.total;
          this.listLoading = false;
        }).catch(() => {
        ...
        columns.forEach((column, index) => {
          ...
          if (!values.every(value => isNaN(value))) {
            if (column.property == 'contractAmount' || column.property == 'amount'
              ...
              || column.property == 'taxBillAmount' || column.property == 'billAmount' || column.property == 'prePayment') {
      ...
      handleSubmit() {
        ...
        //   if (this.sourceBillNo && this.sourceBillNo != '') {
