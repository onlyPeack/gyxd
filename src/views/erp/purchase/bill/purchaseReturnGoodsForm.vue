<template>
  <el-dialog :visible.sync="dialogFormVisible" width="70%" @close="handleClose" class="billFormDialog"
      ...
      <span slot="title" class="con-title">
        ...
        <span v-if="!dataForm.id">新增采购退货申请单</span>
        <span v-if="dataForm.id">编辑采购退货申请单</span>
...
<!--      <div class="association-creation">关联创建</div>-->
    ...
    <el-table :data="items" border highlight-current-row width="100%" :height="clientHeight"
      ...
      <el-table-column prop="skuName" label="商品名称" width="180">
        <template slot-scope="scope">
          <div style="cursor: pointer;">
            <el-input size="mini" suffix-icon="el-icon-more" readonly="readonly" class="readonly-el-input"
      ...
      <el-table-column prop="retailPrice" label="面价" width="180" class-name="col-required">
        <template slot-scope="scope">
          <el-input size="mini" v-model="scope.row.retailPrice" readonly="readonly" class="readonly-el-input"
                    @change="handleEdit(scope.$index, scope.retailPrice , 'retailPrice', $event)"></el-input>
          <span>{{scope.row.retailPrice}}</span>
        </template>
      </el-table-column>
      <el-table-column prop="stockInPurchaseQty" label="入库数量" class-name="col-required">
        <template slot-scope="scope">
          <el-input size="mini" v-model="scope.row.stockInPurchaseQty" readonly="readonly" class="readonly-el-input"></el-input>
          <span>{{scope.row.stockInPurchaseQty}}</span>
        </template>
      </el-table-column>
      <el-table-column prop="returnGoodsQty" label="退货数量" class-name="col-required">
        <template slot-scope="scope">
          <el-input size="mini" v-model="scope.row.returnGoodsQty" readonly="readonly" class="readonly-el-input"></el-input>
      ...
      <el-table-column prop="price" label="单价" width="120" align="right" class-name="col-required">
        <template slot-scope="scope">
          <el-input size="mini" v-model="scope.row.price" class="right-el-input readonly-el-input"  readonly="readonly"
      ...
      <el-table-column prop="amount" label="金额" width="120" align="right" class-name="col-required">
        <template slot-scope="scope">
          <el-input size="mini" v-model="scope.row.amount" class="right-el-input readonly-el-input" readonly="readonly"
      ...
      <el-table-column prop="taxRate" label="税率" width="80" align="center">
        <template slot-scope="scope">
          <el-select v-model="scope.row.taxRate" size="small" placeholder=" " readonly="readonly" class="readonly-el-input"
      ...
      <el-table-column prop="taxPrice" label="含税单价" width="120" align="right" class-name="col-required">
        <template slot-scope="scope">
          <el-input size="mini" v-model="scope.row.taxPrice" class="right-el-input readonly-el-input" readonly="readonly"
      ...
      <el-table-column prop="taxAmount" label="价税合计" width="120" align="right" class-name="col-required">
        <template slot-scope="scope">
          <el-input size="mini" v-model="scope.row.taxAmount" class="right-el-input readonly-el-input" readonly="readonly"
...
<script>
  ...
  import {addBill, getBill, tempAddBill} from '@/api/erp/purchase/bill/purchaseReturnGoods';

  export default {
    name: "purchaseReturnGoodsForm",
    components: {
    ...
    methods: {
      ...
      submitSelectorSuccess(obj) {
        ...
        for (var index in obj.list) {
          ...
          itemObj.stockInPurchaseQty = item.stockInPurchaseQty;
      ...
      checkItemFuc() {
        for (var i in this.items) {
          ...
          if (this.items[i].skuNo && this.items[i].skuNo != '') {
            ...
            if (this.items[i].qty > (this.items[i].stockInPurchaseQty - this.items[i].returnGoodsQty)) {
              this.$message.error('第' + (i * 1 + 1) + '行的数量不能大于入库数量与退货数量之差');
