...
<!--        <span>新增政策资源到账单</span>-->
        <span v-if="!dataForm.id">新增政策资源到账单</span>
        <span v-if="dataForm.id">编辑政策资源到账单</span>
        <!--        <span v-if="dataForm.id">编辑客户订货单</span>-->
          ...
          <tr>
            <td>
              <el-form-item label="返点类型:" prop="rebateType" class="el-form-flex">
                <el-select v-model="dataForm.rebateType">
...
<!--                  <el-option label="特价" value="3"></el-option>-->
            ...
            <td>
              <el-form-item label="供应商:" prop="supplierName" class="el-form-flex">
                <div @click="chooseSupplier(10,false)">
          ...
          <tr>
            <td>
              <el-form-item label="协议单号:" prop="agreementNo" class="el-form-flex">
                <div @click="showPolicyRegisterSelector=true">
                  <el-input v-model="dataForm.agreementNo" placeholder="请选择协议单" >
                    <i class="el-icon-search" style="color:rgb(42,140,254);font-size: 20px;float: right;cursor: pointer;line-height: 32px" slot="suffix"></i>
                  </el-input>
                </div>
              </el-form-item>
          ...
          <tr>
            <td>
              <el-form-item label="返点系列:" prop="companySeries" class="el-form-flex">
                <el-input v-model="dataForm.companySeries" placeholder="请输入返点系列" disabled></el-input>
              </el-form-item>
            </td>
            <td>
              <el-form-item label="协议类型:" prop="sourceBillNo" class="el-form-flex" >
                <el-select v-model="dataForm.agreementType" disabled>
          ...
          <tr>
            <td>
              <el-form-item label="标准系列:" prop="standardSeries" class="el-form-flex">
                <el-input v-model="dataForm.standardSeries" placeholder="请输入标准系列" disabled></el-input>
              </el-form-item>
            </td>
            <td>
              <el-form-item label="协议年度:" prop="agreementYear" class="el-form-flex">
                <el-input v-model="dataForm.agreementYear" placeholder="请输入协议年度" disabled></el-input>
              </el-form-item>
            </td>
          </tr>
          <tr>
            <td>
              <el-form-item label="协议说明:" prop="note" class="el-form-flex">
                <el-input v-model="dataForm.note" placeholder="请输入内容" disabled></el-input>
              </el-form-item>
            </td>
            <td>
              <el-form-item label="票据类型" prop="note" class="el-form-flex">
                <el-select v-model="dataForm.invoiceType">
                  <el-option label="服务费发票" value="1"></el-option>
                  <el-option label="红字发票" value="2"></el-option>
                </el-select>
              </el-form-item>
            </td>
          </tr>
    ...
    <el-table :data="dataForm.items" border highlight-current-row width="100%" :height="clientHeight"
      ...
      <el-table-column prop="applyBillNo" label="请款单号" width="180" align="center" v-if="dataForm.rebateType*1!==2">
        <template slot-scope="scope">
          <div @click.stop="showAssGoods(scope.$index)" style="cursor: pointer;">
            <el-input size="mini" suffix-icon="el-icon-more" v-model="scope.row.applyBillNo" :disabled="dataForm.rebateType*1===2"></el-input>
          </div>
      ...
      <el-table-column prop="receiveBillNo" label="到账单号" width="180" align="center" v-if="dataForm.rebateType*1!==1">
        <template slot-scope="scope">
          <el-input size="mini" v-model="scope.row.receiveBillNo" :disabled="dataForm.rebateType*1===1"></el-input>
          <span>{{scope.row.receiveBillNo}}</span>
        </template>
      </el-table-column>
      <el-table-column label="供应商编号" prop="supplierCode"  align="center" v-slot="{row,$index}">
        <div @click.stop="chooseSupplier($index,true)" style="cursor: pointer;">
          <el-input size="mini" v-model="row.supplierCode" :disabled="dataForm.rebateType*1===1" suffix-icon="el-icon-more"></el-input>
        </div>
        <span>{{row.supplierCode}}</span>
      </el-table-column>
      <el-table-column label="供应商名称" prop="supplierName"  align="center" v-slot="{row}">
        <el-input size="mini" v-model="row.supplierName" :disabled="dataForm.rebateType*1===1"></el-input>
        <span>{{row.supplierName}}</span>
      </el-table-column>
      <el-table-column label="服务费开票单号" prop="invoiceNo"  align="center" v-slot="{row}">
        <el-input size="mini" v-model="row.invoiceNo" :disabled="dataForm.rebateType*1===1"></el-input>
        <span>{{row.invoiceNo}}</span>
      </el-table-column>
      <el-table-column label="开票供应商名称" prop="invoiceSupplier"  align="center" v-slot="{row}">
        <el-input size="mini" v-model="row.invoiceSupplier" :disabled="dataForm.rebateType*1===1"></el-input>
        <span>{{row.invoiceSupplier}}</span>
      </el-table-column>
      <el-table-column label="开票金额" prop="invoiceAmount"  align="center" v-slot="{row}">
        <el-input size="mini" v-model="row.invoiceAmount" :disabled="dataForm.rebateType*1===1"></el-input>
        <span>{{row.invoiceAmount}}</span>
      </el-table-column>
      <el-table-column label="可到款金额" prop="canAmount"  align="center" v-slot="{row}">
        <el-input size="mini" v-model="row.canAmount" disabled></el-input>
        <span>{{row.canAmount}}</span>
      </el-table-column>
      <el-table-column label="本次到款金额" prop="thisAmount"  align="center" v-slot="{row}">
        <el-input size="mini" v-model="row.thisAmount" @input="computeThisAmount(row)"></el-input>
...
<!--      </div>-->
    ...
    <!--    付款申请单选择框-->
    <payment-apply-selector v-if="showPaymentApplySelector" :dialogVisible="showPaymentApplySelector" @handleClose="handleClosePublic" @handleSubmit="handleSubmitPublic"></payment-apply-selector>
...
<script>
  ...
  import {addBill,tempAddBill,getBill} from '@/api/erp/purchase/bill/policyEffect';
  ...
  export default {
    ...
    components: {
      ...
      'paymentApplySelector': () => import('@/components/ERP/Purchase/paymentApplySelector'),
    },
    ...
    data() {
      return {
        ...
        showPaymentApplySelector:false,//是否显示付款申请单关联对话框
        isRow:false,//是否在行内选择供应商
    ...
    methods: {
        ...
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            for (let i = 0; i <this.dataForm.items.length ; i++) {
              if((!this.dataForm.items[i].applyBillNo||this.dataForm.items[i].applyBillNo==='')&&(!this.dataForm.items[i].receiveBillNo||this.dataForm.items[i].receiveBillNo==='')){
                this.$message.warning('请填写第'+(i+1)+'行的到账单号或请款单号！')
                return false
              }
            }
            addBill(this.dataForm).then(res => {
              if (Number(res.code) === 2000 || Number(res.code) === 200) {
                let ids = {
                  type: 'policyEffectForm',
                  flag: true
                };
                this.$emit('submitSuccess', ids);
              } else {
                if (Number(res.code) === 401) {
                  this.$message.warning('创建政策资源到账单失败,' + res.msg || res.data)
                }
                {
                  this.$message.error('创建政策资源到账单失败,' + res.msg || res.data)
                }
              }
            }, error => this.$message.error('创建政策资源到账单失败,' + error))
          }
        })
      },
      handleTempAddBill() {
        ...
        tempAddBill(this.dataForm).then(res=>{
          if(Number(res.code)===2000||Number(res.code)===200){
            ...
            if(Number(res.code)===401){
              this.$message.warning('暂存政策资源到账单失败,'+res.msg||res.data)
            }{
              this.$message.error('暂存政策资源到账单失败,'+res.msg||res.data)
            }
          }
        },error=>this.$message.error('暂存政策资源到账单失败,'+error))
      },
      checkItemFuc() {
      ...
      handleClosePublic() {
        ...
        this.showPaymentApplySelector=false
      ...
      handleSubmitPublic(list){
        if(list.type==='policyChannelType'){
          ...
          this.showSupplierSelector=false
          if(this.isRow){
            this.$set(this.dataForm.items[this.clickRow],'supplierCode',list.arr.supplierCode)
            this.$set(this.dataForm.items[this.clickRow],'supplierName',list.arr.supplierName)
          }else{
          ...
          this.showBrandSelector=false
          this.$set(this.dataForm,'brand',list.arr.name)
          this.$refs['dataForm'].validateField('brand')
          ...
          this.dataForm.standardSeries=list.arr.standardSeries
          this.dataForm.agreementNo=list.arr.billNo
          this.dataForm.note=list.arr.note
        }else if(list.type==='paymentApply'){
          console.log(list.arr,'付款申请单数据')
          this.showPaymentApplySelector=false
          this.$set(this.dataForm.items[this.clickRow],'applyBillNo',list.arr.billNo)
          this.$set(this.dataForm.items[this.clickRow],'supplierCode',list.arr.supplierCode)
          this.$set(this.dataForm.items[this.clickRow],'supplierName',list.arr.supplierName)
          this.$set(this.dataForm.items[this.clickRow],'invoiceSupplier',list.arr.supplierName)
          this.$set(this.dataForm.items[this.clickRow],'invoiceAmount',list.arr.applyAmount)
          this.$set(this.dataForm.items[this.clickRow],'canAmount',this.accAdd(list.arr.applyAmount,-list.arr.paidAmount))
          this.$set(this.dataForm.items[this.clickRow],'thisAmount',this.accAdd(list.arr.applyAmount,-list.arr.paidAmount))
          this.$set(this.dataForm.items[this.clickRow],'note',list.arr.note)
        }
      },
       ...
       */
      showAssGoods(index){
        if(this.dataForm.rebateType*1===2){
          return false
        }
        this.showPaymentApplySelector=true
        this.clickRow=index
      },

      chooseSupplier(index,flag){
        this.isRow=flag
        if(flag){
          this.clickRow=index
        }
        this.showSupplierSelector=true
      },
       ...
       * @param row 行数据
       */
      computeThisAmount(row){
        if(row.thisAmount>row.canAmount){
          this.$message.warning('本次到款金额不能大于可到款金额!')
          row.thisAmount=row.canAmount
          return false
        }
      }
    }
