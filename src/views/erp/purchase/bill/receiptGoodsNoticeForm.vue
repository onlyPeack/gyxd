...
<!--        <span v-if="!dataForm.id">新增收货通知单</span>-->
<!--        <span v-if="dataForm.id">编辑收货通知单</span>-->
...
<!--      <div class="association-creation">关联创建</div>-->
      ...
      <el-form :model="dataForm" :rules="rules" ref="dataForm" size="medium " label-width="120px" label-position="right">
        <table class="topTable" style="width: 100%" cellspacing="0">
          <tr>
            ...
            <td class="from-item no-border-bottom">
              <el-form-item label="供应商:" prop="supplierName" class="el-form-flex">
                <div @click="handleSupplierSelector" style="cursor: pointer;line-height: 36px;margin-right: 15px;">
                  <span>
                    <svg-icon icon-class="add"></svg-icon>
                  </span>
          ...
          <tr>
            ...
            <td class="from-item no-border-right ">
              <el-form-item label="发货日期:" prop="deliverTime" class="el-form-flex">
                <el-date-picker v-model="dataForm.deliverTime"  :clearable="false" align="right" type="datetime" value-format="yyyy-MM-dd HH:mm:ss"
                                style="width: 200px" placeholder="请选择日期">
                </el-date-picker>
              </el-form-item>
            </td>
            <td class="from-item">
              <el-form-item label="预计到货日期:" prop="expectArrivalDate" class="el-form-flex">
                <el-date-picker v-model="dataForm.expectArrivalDate"   :clearable="false" align="right" type="datetime" value-format="yyyy-MM-dd HH:mm:ss"
                                style="width: 200px" placeholder="请选择日期">
                </el-date-picker>
              </el-form-item>
            </td>
          ...
          <tr>
            <td class="from-item no-border-right no-border-bottom">
              <el-form-item label="到货仓库:" prop="warehouseName" @click="handleWarehouseSelector" class="el-form-flex">
                <div style="line-height: 40px;margin-right: 15px;">
                  <span @click="handleWarehouseSelector">
                    <svg-icon icon-class="add"></svg-icon>
                  </span>
          ...
          <tr>
            <td colspan="4" class="from-item no-border-bottom">
...
<!--          <span>{{scope.row.seriesName}}</span>-->
      ...
      <el-table-column prop="rebateSeries" label="返点系列" width="180">
        <template slot-scope="scope">
          <el-input size="mini" v-model="scope.row.rebateSeries" readonly="readonly" class="readonly-el-input"
                    @change="handleEdit(scope.$index, scope.row , 'rebateSeries', $event)"></el-input>
...
<!--          <span>{{scope.row.itemNo}}</span>-->
      ...
      <el-table-column prop="unit" label="单位" width="80">
        <template slot-scope="scope">
          <el-input size="mini" v-model="scope.row.unit" readonly="readonly" class="readonly-el-input"
                    @change="handleEdit(scope.$index, scope.row , 'unit', $event)"></el-input>
<!--          <span>{{scope.row.unit}}</span>-->
        </template>
      </el-table-column>
      <el-table-column prop="canGoodsQty" label="可做收货数量" width="120">-->
        <template slot-scope="scope">
          <el-input size="mini" v-model="scope.row.canGoodsQty" readonly="readonly" class="readonly-el-input"
                    @change="handleEdit(scope.$index, scope.row , 'canGoodsQty', $event)"></el-input>
...
<!--          <span>{{scope.row.taxAmount}}</span>-->
      ...
      <el-table-column prop="packingNumber" label="装箱单号" width="300">
        <template slot-scope="scope">
          <el-input size="mini" v-model="scope.row.packingNumber"></el-input>
        </template>
      </el-table-column>
...
<!--    </div>-->
    ...
    <purchase-order-selector :dialogSelectorVisible="dialogSelectorVisible" v-if="dialogSelectorVisible"
                             ...
                             @handleSelectorClose="handleSelectorClose" :sourceBillNo="dataForm.sourceBillNo" :type="6">
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      //   this.$set(this.dataForm,'expectArrivalDate',this.$moment(val).format("YYYY-MM-DD HH:mm:ss"))
      // },
      // handleMainDeliverDateChange(val) {
      //   this.$set(this.dataForm,'deliverTime',this.$moment(val).format("YYYY-MM-DD HH:mm:ss"))
      // },
      // handleBillDateChange(val) {
      //   this.$set(this.dataForm,'billDate',this.$moment(val).format("YYYY-MM-DD HH:mm:ss"))
      // },
      changeDivHeight() {
        ...
        this.clientHeight = (document.body.clientHeight - (180 + height)) * 0.95
      },
      ...
      init() {
        if (this.id != '') {
          ...
          this.$set(this.dataForm,'expectArrivalDate',new Date())
          this.$set(this.dataForm,'deliverTime',new Date())
      ...
      handleEdit(index, row, type, val) {
        ...
        if (row.qty && row.qty != '') {
          if (type == 'qty' || type == 'price' || type == 'taxRate') {
            console.log('测试');
            console.log(row);
            if(Number(row.qty) > Number(row.canGoodsQty)) {
              this.$message.warning('数量不能大于可做收货数量!')
              this.vueSet(this.items[index], 'qty', row.canGoodsQty)
            }
        ...
        for (var index in obj.list) {
          ...
          if (index == 0) {
            if (item.warehouseName) {
              ...
              _this.dataForm.warehouseArr = [{warehouseName: item.warehouseName, warehouseId: item.warehouseId}]
            ...
            this.$set(_this.dataForm,'sourceBillNo',item.mainDTO.billNo)
            this.$set(_this.dataForm,'supplierName',item.mainDTO.supplierName)
          ...
          itemObj.specifications = item.specifications
          itemObj.unit = item.unit
          itemObj.price = Number(item.price).toFixed(4)
          itemObj.skuName = item.skuName
          itemObj.rebateSeries = item.rebateSeries
          itemObj.canGoodsQty = item.canGoodsQty + ''
          itemObj.qty = item.notStockInPurchaseQty > item.canGoodsQty ? item.canGoodsQty : item.notStockInPurchaseQty
          console.log('可做收货数量');
          console.log(itemObj.canGoodsQty);
          itemObj.amount = Number(item.amount).toFixed(4)
          itemObj.taxRate = item.taxRate + ''
          itemObj.tax = item.tax.toFixed(4)
          itemObj.taxPrice = Number(item.taxPrice).toFixed(4)
          itemObj.taxAmount = Number(item.taxAmount).toFixed(4)
      ...
      checkItemFuc() {
        for (var i in this.items) {
          ...
          if (this.items[i].skuNo && this.items[i].skuNo != '') {
            if(Number(this.items[i].qty) > Number(this.items[i].canGoodsQty)) {
              this.$message.error('第' + (i * 1 + 1) + '行的数量不能大于可做收货数量，请重新填写')
              return
            }
