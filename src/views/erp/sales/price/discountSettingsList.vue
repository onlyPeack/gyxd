<template>
  <div class="full-purchase bill-container">
    <el-tabs v-model="activeTabName" @tab-click="handleTabClick">
      <el-tab-pane label="折扣设置查询" name="second">
        ...
        <div v-if="activeTabName=='second'">
          ...
          <el-table :data="list" border style="width: 100%" v-loading="listLoading" ref="dataTable" height="60vh"
            ...
            <el-table-column prop="customerNames" label="客户名称" sortable>
              <template slot-scope="scope">
            ...
            <el-table-column align="center" label="客户等级" prop="level">
              <template slot-scope="scope">
                <span v-for="(item,index) in levelOptions" :key="index">
                  <span v-if="scope.row.customerArr[0].level ===item.value">{{item.label}}</span>
                </span>
              </template>
            </el-table-column>
      ...
      <el-tab-pane :label="items[0] && items[0].id ? '编辑折扣设置' : '新增折扣设置'" name="first">
        <div class="billFormDialog" style="width: 1000px;" v-if="activeTabName=='first'">
          <el-table :data="items" border highlight-current-row class="tb-edit">
            ...
            <el-table-column prop="customerName" label="客户名称" min-width="300" align="left" class="customer-tag">
              <template slot-scope="scope">
      ...
      <el-tab-pane v-if="$store.state.user.code == '00000'" :label="items[0] && items[0].id ? '编辑所有客户折扣' : '新增所有客户折扣'" name="tree">
...
<script>
  import { addBill, selectPage, getBill, batchDelBill } from '@/api/erp/sales/price/discountSetting'
      ...
      checkItemFuc() {
        for (var i in this.items) {
          if (!this.items[i].customerArr || this.items[i].customerArr.length == 0) {
            this.$message.error('第' + (i * 1 + 1) + '行的适用客户为必填项，请填写了再提交')
            return
          }
