<template>
  <el-dialog :visible.sync="dialogFormVisible" width="70%" @close="handleClose" class="billFormDialog"
      ...
      <span slot="title" class="con-title">
        ...
        <span v-if="!dataForm.id">新增客户价格方案</span>
        <span v-if="dataForm.id">编辑客户价格方案</span>
        <span class="cont-title-billNo" v-if="dataForm.billNo">No.{{dataForm.billNo}}</span>
      </span>

    <div class="center-container">
      <div class="association-creation">
        <el-upload class="upload"  v-if="dataForm.type != 2 && dataForm.type != 3 && dataForm.type != 4 && dataForm.type != 5" action="/api/sales/sales-pricing-scheme/import" :before-upload="beforeUpload"
        ...
        <el-button type="text" icon="el-icon-download" v-if="dataForm.type != 2 && dataForm.type != 3 && dataForm.type != 4 && dataForm.type != 5" >
            ...
            <td>
              <el-form-item label="适用客户:" prop="customerName" class="el-form-flex">
                ...
                <span>
                  ...
                  <el-tag v-for="(tag,tagIndex) in dataForm.customerArr" :key="tag.id" closable
    ...
    <el-table :data="items" border highlight-current-row width="100%" :height="clientHeight"
      ...
      <el-table-column prop="skuName" label="商品名称" min-width="180" v-if="dataForm.type != 2 && dataForm.type != 3 && dataForm.type != 4 && dataForm.type != 5">
        <template slot-scope="scope">
      ...
      <el-table-column prop="seriesName" label="系列名称" width="380" v-if="dataForm.type == 2 || dataForm.type == 4">
        <template slot-scope="scope">
          <div @click.stop="handleSeriesNameShow(scope.$index)" style="cursor: pointer;">
            <el-input size="mini" suffix-icon="el-icon-more" v-model="scope.row.seriesName"
                      @keyup.native="changeFocusInput($event,scope.$index,scope.$index*tableElInputNum+1)"
                      :ref="'tableElInput'+(scope.$index*tableElInputNum+1)"></el-input>
          </div>
          <span>{{scope.row.skuName}}</span>
      ...
      <el-table-column prop="brandName" label="品牌名称" width="380" v-if="dataForm.type == 3 || dataForm.type == 5">
        <template slot-scope="scope">
          <div @click.stop="handlebrandNameShow(scope.$index)" style="cursor: pointer;">
            <el-input size="mini" suffix-icon="el-icon-more" v-model="scope.row.brandName"
                      @keyup.native="changeFocusInput($event,scope.$index,scope.$index*tableElInputNum+1)"
                      :ref="'tableElInput'+(scope.$index*tableElInputNum+1)"></el-input>
          </div>
      ...
      <el-table-column prop="brandName" label="品牌" width="180" align="center" v-if="dataForm.type != 2 && dataForm.type != 3 && dataForm.type != 4 && dataForm.type != 5">
        <template slot-scope="scope">
      ...
      <el-table-column prop="seriesName" label="系列" width="180" align="center" v-if="dataForm.type != 2 && dataForm.type != 3 && dataForm.type != 4 && dataForm.type != 5">
        <template slot-scope="scope">
      ...
      <el-table-column prop="specifications" label="规格型号" width="180" align="center" v-if="dataForm.type != 2 && dataForm.type != 3 && dataForm.type != 4 && dataForm.type != 5">
        <template slot-scope="scope">
      ...
      <el-table-column prop="itemNo" label="订货号" width="180" align="center" v-if="dataForm.type != 2 && dataForm.type != 3 && dataForm.type != 4 && dataForm.type != 5">
        <template slot-scope="scope">
      ...
      <el-table-column prop="price" label="面价" width="80" align="right" v-if="dataForm.type != 2 && dataForm.type != 3 && dataForm.type != 4 && dataForm.type != 5">
        <template slot-scope="scope">
      ...
      <el-table-column prop="unit" label="单位" width="80" align="center" v-if="dataForm.type != 2 && dataForm.type != 3 && dataForm.type != 4 && dataForm.type != 5">
        <template slot-scope="scope">
      ...
      <el-table-column prop="customerDiscount" label="客户订货折扣" v-if="dataForm.type != 2 && dataForm.type != 3 && dataForm.type != 4 && dataForm.type != 5" width="120" align="right" class-name="col-required">
        <template slot-scope="scope">
          <el-input size="mini" v-model="scope.row.customerDiscount" class="right-el-input"
                    @input="handleInputFun($event, scope.row, 'customerDiscount')"
                    @change="handleEdit(scope.$index, scope.row , 'customerDiscount', $event)"
      ...
      <el-table-column prop="customerDiscount" label="客户订货折扣" v-if="dataForm.type == 2 || dataForm.type == 3 || dataForm.type == 4 || dataForm.type == 5" width="380" align="right" class-name="col-required">
        <template slot-scope="scope">
          <el-input size="mini" v-model="scope.row.customerDiscount" class="right-el-input"
                    @input="handleInputFun($event, scope.row, 'customerDiscount')"
                    @change="handleEdit(scope.$index, scope.row , 'customerDiscount', $event)"
      ...
      <el-table-column prop="customerPrice" label="客户订货价" width="150" align="right" v-if="dataForm.type != 2 && dataForm.type != 3 && dataForm.type != 4 && dataForm.type != 5" class-name="col-required">
        <template slot-scope="scope">
          <el-input size="mini" v-model="scope.row.customerPrice" class="right-el-input"
                    @input="handleInputFun($event, scope.row, 'customerPrice')"
                    @change="handleEdit(scope.$index, scope.row , 'customerPrice', $event)"
      ...
      <el-table-column prop="referencePrice" label="参考价格" width="150" align="right" v-if="dataForm.type != 2 && dataForm.type != 3 && dataForm.type != 4 && dataForm.type != 5">
        <template slot-scope="scope">
          <el-input size="mini" v-model="scope.row.referencePrice" class="readonly-el-input right-el-input"
                    readonly="readonly"></el-input>
          <span>{{scope.row.referencePrice}}</span>
        </template>
      </el-table-column>
      <el-table-column prop="customerDifference" label="毛利率" width="150" align="right" v-if="dataForm.type != 2 && dataForm.type != 3 && dataForm.type != 4 && dataForm.type != 5">
        <template slot-scope="scope">
          <el-input size="mini" v-model="scope.row.customerDifference" class="readonly-el-input right-el-input"
                    readonly="readonly"></el-input>
          <span>{{scope.row.customerDifference}}</span>
        </template>
      </el-table-column>
      <el-table-column prop="safeDiscount" label="安全折扣" width="120" align="right" v-if="dataForm.type != 2 && dataForm.type != 3 && dataForm.type != 4 && dataForm.type != 5">
        <template slot-scope="scope">
          <el-input size="mini" v-model="scope.row.safeDiscount" class="readonly-el-input right-el-input"
                    readonly="readonly"></el-input>
          <span>{{scope.row.safeDiscount}}</span>
        </template>
      </el-table-column>
      <el-table-column prop="safePrice" label="安全价格" width="150" align="right" v-if="dataForm.type != 2 && dataForm.type != 3 && dataForm.type != 4 && dataForm.type != 5">
        <template slot-scope="scope">
          <el-input size="mini" v-model="scope.row.safePrice" class="readonly-el-input right-el-input"
                    readonly="readonly"></el-input>
    ...
    <!-- 系列弹窗-->
    <el-dialog title="请选择系列" width="50%" :visible.sync="seriesVisible" append-to-body>
...
<script>
  ...
  import {addBill, getBill, tempAddBill} from '@/api/erp/sales/price/pricePlan';
  ...
  export default {
    ...
    components: {
      ...
      'brandSelector': () => import('@/views/erp/sales/components/brandSelector'),
    ...
    methods: {
      handleSubmitPublic(list) {
        ...
        this.items[0].brandName = list[0].name;
        this.vueSet(this.items, this.scopeIndex, this.items[0]);
        console.log('items');
      ...
      closeSeriesDialog(list) {
        if (list.length > 0) {
          ...
          if (this.dataForm.type == 2 || this.dataForm.type == 4) {
            this.items[this.scopeIndex].seriesName = list[0].name;
            this.items[this.scopeIndex].brandName = list[0].brandName;
        ...
        _this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            ...
            for (var i in _this.items) {
              ...
              } else if(this.dataForm.type == '3' || this.dataForm.type == '5') {
                if (_this.items[i].brandName && _this.items[i].brandName != '') {
                  list.push(_this.items[i])
                }
              }else{
