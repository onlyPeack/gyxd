<template>
  <div class="full-purchase bill-container">
    ...
    <el-table :data="list" border style="width: 100%" v-loading="listLoading" ref="dataTable"
      ...
      <el-table-column prop="mainDTO.schemeName" label="价格方案" min-width="250">
        <template slot-scope="scope">
          <div class="colum-billno" @click="handleOpenDetailOrForm(scope.row)" v-if="scope.row.mainDTO">
      ...
      <el-table-column prop="mainDTO.customerName" label="适用客户" width="250">
        <template slot-scope="scope" v-if="scope.row.mainDTO && scope.row.mainDTO.customerArr">
          <span v-if="scope.row.mainDTO.customerArr.length>0" v-for="(item,index) in scope.row.mainDTO.customerArr">
              {{item.customerName}}
              <span v-if="index!=(scope.row.mainDTO.customerArr.length-1)">,</span>
          </span>
        </template>
      </el-table-column>
      <el-table-column prop="customerPrice" label="客户订货价" align="right" width="150" class-name="col-required">
      </el-table-column>
      <el-table-column prop="customerDiscount" label="客户订货折扣" align="right" width="150" class-name="col-required">
      </el-table-column>
    ...
    <el-table :data="list" border style="width: 100%" v-loading="listLoading" ref="dataTable"
      ...
      <el-table-column prop="schemeName" label="单据号">
        <template slot-scope="scope">
      ...
      <el-table-column prop="customerName" label="适用客户">
        <template slot-scope="scope">
          <span v-if="scope.row.customerArr.length>0" v-for="(item,index) in scope.row.customerArr">
              {{item.customerName}}
              <span v-if="index!=(scope.row.customerArr.length-1)">,</span>
          </span>
        </template>
      </el-table-column>
    ...
    <!-- 新增价格方案弹窗 -->
    <price-plan-form :dialogFormVisible="dialogFormVisible" v-if="dialogFormVisible" :id="id"
                     @submitSuccess="submitSuccess" @handleClose="handleClose" :typeOptions="typeOptions">
    </price-plan-form>

    <!-- 价格方案详情弹窗 -->
    <price-plan-view :dialogFormVisible="dialogDetailVisible" v-if="dialogDetailVisible" :id="id"
...
<script>
  import {selectPage, selectPageByItem, batchDelBill} from '@/api/erp/sales/price/pricePlan';
  import {getFloat} from '@/utils/util';

  export default {
    name: "pricePlanList",
    components: {
      'price-plan-form': () => import('./pricePlanForm'),
      'price-plan-view': () => import('./pricePlanView')
    },
    data() {
      return {
