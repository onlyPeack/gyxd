<template>
  <div class="app-container calendar-list-container sales-summary-list-container">
    ...
    <div class="filter-container sales-summary-list-filter-container">
      <div>
        ...
        <el-input clearable class="filter-item" style="width: 200px;" placeholder="商品名称"
                  @keyup.enter.native="handleFilter" v-model="listQuery.skuName" v-if="listQuery.activeName=='skuNo'">
        </el-input>
        <el-input clearable class="filter-item" style="width: 200px;" placeholder="订货号"
                  @keyup.enter.native="handleFilter" v-model="listQuery.itemNo" v-if="listQuery.activeName=='skuNo'">
        </el-input>
        <el-input clearable class="filter-item" style="width: 200px;" placeholder="规格型号"
                  ...
                  v-if="listQuery.activeName=='skuNo'">
        </el-input>
    ...
    <el-table size="small" :data="list" v-loading="listLoading" element-loading-text="正在查询中。。。" border fit
      ...
      <el-table-column align="right" label="出库成本价" prop="stockOutCostPrice" sortable>
      </el-table-column>
      <el-table-column align="right" label="出库成本金额" prop="stockOutCostAmount" sortable>
      </el-table-column>
      ...
      <el-table-column align="right" label="退库成本金额" prop="returnCostAmount" sortable>
      </el-table-column>
      <el-table-column align="right" label="毛利" prop="grossMargin" sortable>
      </el-table-column>
      <el-table-column align="right" label="毛利率" prop="grossMarginRate">
      </el-table-column>
    </el-table>
    <!--按分类-->
    <el-table size="small" :data="list" v-loading="listLoading" element-loading-text="正在查询中。。。" border fit
    ...
    methods: {
      handleDownloadByActiveName() {
        if (this.listQuery.activeName == 'skuNo') {
          ...
          import('@/vendor/Export2Excel').then(excel => {
            const tHeader = ['商品', '品牌名称', '订货号', '型号', '单位', '出库单价', '出库数量', '出库金额', '出库成本价', '出库成本金额', '退库数量', '退库金额', '退库成本金额', '毛利', '毛利率'];
            const filterVal = ['skuName', 'brandName', 'itemNo', 'specifications', 'unit', 'stockOutPrice', 'stockOutQty', 'stockOutAmount', 'stockOutCostPrice', 'stockOutCostAmount', 'returnStockQty', 'returnStockAmount', 'returnCostAmount', 'grossMargin', 'grossMarginRate'];
            excel.export_json_to_excel2(tHeader, this.list, filterVal, '销售利润统计-按商品');
            this.downloadLoading = false;
          })
        } else if (this.listQuery.activeName == 'categoryId') {
          this.downloadLoading = true;
          import('@/vendor/Export2Excel').then(excel => {
            const tHeader = ['分类', '单位', '出库单价', '出库数量', '出库金额', '出库成本价', '出库成本金额', '退库数量', '退库金额', '退库成本金额', '毛利', '毛利率'];
            const filterVal = ['categoryName', 'unit', 'stockOutPrice', 'stockOutQty', 'stockOutAmount', 'stockOutCostPrice', 'stockOutCostAmount', 'returnStockQty', 'returnStockAmount', 'returnCostAmount', 'grossMargin', 'grossMarginRate'];
            excel.export_json_to_excel2(tHeader, this.list, filterVal, '销售利润统计-按分类');
            this.downloadLoading = false;
          })
        } else if (this.listQuery.activeName == 'customer') {
          this.downloadLoading = true;
          import('@/vendor/Export2Excel').then(excel => {
            const tHeader = ['客户名称', '客户编码', '出库数量', '出库金额', '出库成本金额', '退库数量', '退库金额', '退库成本金额', '毛利', '毛利率'];
            const filterVal = ['customerName', 'customerCode', 'stockOutQty', 'stockOutAmount', 'stockOutCostAmount', 'returnStockQty', 'returnStockAmount', 'returnCostAmount', 'grossMargin', 'grossMarginRate'];
            excel.export_json_to_excel2(tHeader, this.list, filterVal, '销售利润统计-按客户');
            this.downloadLoading = false;
          })
        } else if (this.listQuery.activeName == 'departId') {
          this.downloadLoading = true;
          import('@/vendor/Export2Excel').then(excel => {
            const tHeader = ['部门', '出库数量', '出库金额', '出库成本金额', '退库数量', '退库金额', '退库成本金额', '毛利', '毛利率'];
            const filterVal = ['departName', 'stockOutQty', 'stockOutAmount', 'stockOutCostAmount', 'returnStockQty', 'returnStockAmount', 'returnCostAmount', 'grossMargin', 'grossMarginRate'];
            excel.export_json_to_excel2(tHeader, this.list, filterVal, '销售利润统计-按部门');
            this.downloadLoading = false;
          })
        } else if (this.listQuery.activeName == 'salerId') {
          this.downloadLoading = true;
          import('@/vendor/Export2Excel').then(excel => {
            const tHeader = ['业务员工号', '业务员姓名', '出库数量', '出库金额', '出库成本金额', '退库数量', '退库金额', '退库成本金额', '毛利', '毛利率'];
            const filterVal = ['salerCode', 'salerName', 'stockOutQty', 'stockOutAmount', 'stockOutCostAmount', 'returnStockQty', 'returnStockAmount', 'returnCostAmount', 'grossMargin', 'grossMarginRate'];
            excel.export_json_to_excel2(tHeader, this.list, filterVal, '销售利润统计-按业务员');
            this.downloadLoading = false;
          });
        }
      },
      ...
      init() {
        listCatAndBrand().then(response => {
          if (response.data) {
            this.categoryList = response.data.categoryList ? response.data.categoryList : [];
      ...
      getList() {
        ...
        selectPage(this.listQuery).then(response => {
          ...
          this.grossMargin = 0;
          this.stockOutAmount = 0;
          for (var i in this.list) {
            this.grossMargin = this.grossMargin + this.list[i].grossMargin;
            this.stockOutAmount = this.stockOutAmount + this.list[i].stockOutAmount;
          }
          this.grossMargin = Number(getFloat(this.grossMargin, 2));
          this.stockOutAmount = Number(getFloat(this.stockOutAmount, 2));
      ...
      getSummaries(param) {
        ...
        columns.forEach((column, index) => {
          ...
          let values = [];
          if (column.property === 'grossMarginRate') {
            values = data.map(item => Number(parseFloat(item.grossMarginRate) / 100));
          } else {
            values = data.map(item => Number(item[column.property]));
          }
          if (!values.every(value => isNaN(value))) {
            if (column.property == 'stockOutQty' || column.property == 'stockOutAmount' || column.property == 'grossMargin' || column.property == 'grossMarginRate'
              || column.property == 'stockOutCostAmount' || column.property == 'returnStockQty' || column.property == 'returnStockAmount'
              || column.property == 'returnCostAmount') {
              sums[index] = values.reduce((prev, curr) => {
                const value = Number(curr);
                if (!isNaN(value)) {

                  if (column.property == 'grossMarginRate') {
                    return this.avgGrossMarginRate();
                  }
      ...
      avgGrossMarginRate() {
        return getFloat(getFloat(this.grossMargin / this.stockOutAmount, 6) * 100, 4) + '%';
      }
