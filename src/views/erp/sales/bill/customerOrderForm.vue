<template>
  <el-dialog :visible.sync="dialogFormVisible" width="70%" @close="handleClose" class="billFormDialog"
      ...
      <span slot="title" class="con-title">
        ...
        <span v-if="!dataForm.id">新增客户订货单</span>
...
<!--      <div class="association-creation">关联创建</div>-->
            ...
            <td>
              <el-form-item label="交货日期:" prop="deliverDate" class="el-form-flex">
                <el-date-picker v-model="dataForm.deliverDate" align="right" type="date"
                                style="width: 200px" placeholder="请选择日期" @change="handleDeliverDateChange">
                </el-date-picker>
              </el-form-item>
            </td>
            <td>
          ...
          <tr>
            <td>
              <el-form-item label="收款条件:" prop="receiptCondition" class="el-form-flex">
                <el-select v-model="dataForm.receiptCondition" style="width: 100%;">
    ...
    <!-- 关联单据 -->
    <sales-contract-selector :dialogSelectorVisible="dialogSelectorVisible" v-if="dialogSelectorVisible"
                             @submitSelectorSuccess="submitSelectorSuccess"
                             @handleSelectorClose="handleSelectorClose" :sourceBillNo="dataForm.sourceBillNo">
    </sales-contract-selector>
  </el-dialog>
</template>

<script>
  import {getFloat} from '@/utils/util'
  import {addBill, getBill, tempAddBill} from '@/api/erp/sales/bill/salesOrder'
  ...
  export default {
    ...
    components: {
      ...
      'sales-contract-selector': () => import('../components/salesContractSelector')
    },
