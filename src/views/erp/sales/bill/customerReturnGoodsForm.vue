<template>
  <el-dialog :visible.sync="dialogFormVisible" width="70%" @close="handleClose" class="billFormDialog"
      ...
      <span slot="title" class="con-title">
        ...
        <span v-if="!dataForm.id">新增客户退货单</span>
        <span v-if="dataForm.id">编辑客户退货单</span>
          ...
          <tr>
            ...
            <td>
              <el-form-item label="退货地址:" prop="linkAddress" class="el-form-flex">
                <el-input v-model="dataForm.linkAddress" placeholder="请输入退货地址"></el-input>
              </el-form-item>
            </td>
          </tr>
          <tr>
    ...
    <!-- 退货单位列表弹窗-->
    <el-dialog title="请选择退货单位" :visible.sync="customerVisible" append-to-body>
...
<script>
  ...
  import {addBill, getBill, tempAddBill} from '@/api/erp/sales/bill/salesReturnGoods';
  import {getSelfCompany} from '@/api/admin/company';
  import {page} from '@/api/erp/crm/customer/index';

  export default {
    name: "salesReturnGoodsForm",
    components: {
    ...
    methods: {
      ...
      closeCustomerDialog(list) {
        ...
        if (list.length > 0) {
          ...
          this.dataForm.linkPerson = list[0].linkMan ? list[0].linkMan : '';
          this.dataForm.linkPhone = list[0].linkPhone ? list[0].linkPhone : '';
          this.dataForm.linkAddress = list[0].linkAddress ? list[0].linkAddress : '';
        }
      },
      ...
      submitSelectorSuccess(obj) {
        ...
        var listQuery = {
          customerCode: obj.list[0].mainDTO.customerCode,
          page: 1,
          limit: 20,
        }
        page(listQuery)
          .then(response => {
            this.dataForm.linkPerson = response.data.records[0].linkMan ? response.data.records[0].linkMan : '';
            this.dataForm.linkPhone = response.data.records[0].linkPhone ? response.data.records[0].linkAddress : '';
            this.dataForm.linkAddress = response.data.records[0].linkAddress ? response.data.records[0].linkAddress : '';
          })
