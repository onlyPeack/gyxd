<template>
  <div class="full-purchase bill-container">
    ...
    <el-table :data="list" border style="width: 100%" v-loading="listLoading" ref="dataTable"
      ...
      <el-table-column prop="companyName" label="供货单位" width="150">
      </el-table-column>
      <el-table-column prop="customerName" label="退货单位" width="150">
      </el-table-column>
      <el-table-column label="币别" width="80">
        <template slot-scope="scope">
            <span v-for="item in currencyOptions">
    ...
    <!-- 新增客户退货单弹窗 -->
    <customer-return-goods-form :dialogFormVisible="dialogFormVisible" v-if="dialogFormVisible" :id="id"
                                @submitSuccess="submitSuccess" @handleClose="handleClose">
    </customer-return-goods-form>

    <!-- 客户退货单详情弹窗 -->
    <customer-return-goods-view :dialogFormVisible="dialogDetailVisible" v-if="dialogDetailVisible" :id="id"
                                @submitDetailSuccess="submitDetailSuccess" @handleClose="handleDetailClose">
    </customer-return-goods-view>
  </div>
</template>

<script>
  import {selectPage, selectPageByItem, batchDelBill} from '@/api/erp/sales/bill/salesReturnGoods';
  import {getFloat} from '@/utils/util';

  export default {
    name: "salesReturnGoodsList",
    components: {
      'customer-return-goods-form': () => import('./customerReturnGoodsForm'),
      'customer-return-goods-view': () => import('./customerReturnGoodsView')
    },
    data() {
      return {
      ...
      handleDownload() {
        ...
        import('@/vendor/Export2Excel').then(excel => {
          ...
          if (this.choose == false) {
            tHeader = ['单据日期', '单据编号', '制单人', '业务员', '供货单位', '退货单位', '币别', '金额', '价税合计', '单据状态', '开票状态'];
            filterVal = ['crtTime', 'billNo', 'crtUserName', 'salerName', 'companyName', 'customerName', 'currencyId', 'billAmount', 'taxBillAmount', 'billStatus', 'invoiceStatus'];
          } else {
            for (var i in list) {
              Object.assign(list[i], list[i].mainDTO);
              delete list[i].mainDTO;
            }
            tHeader = ['单据日期', '单据编号', '制单人', '业务员', '供货单位', '退货单位', '币别', '金额', '价税合计', '单据状态', '开票状态', '商品编码', '商品名称', '单位', '数量', '单价', '金额', '税额', '含税单价', '价税总计', '开票数量'];
            filterVal = ['crtTime', 'billNo', 'crtUserName', 'salerName', 'companyName', 'customerName', 'currencyId', 'billAmount', 'taxBillAmount', 'billStatus', 'invoiceStatus', 'skuNo', 'skuName', 'unit', 'qty', 'price', 'amount', 'tax', 'taxPrice', 'taxAmount', 'purchaseInvoiceQty'];
          }
          excel.export_json_to_excel2(tHeader, list, filterVal, '客户退货单');
