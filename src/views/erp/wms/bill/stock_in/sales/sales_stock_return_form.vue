<template>
  <el-dialog :visible.sync="dialogFormVisible" width="70%" @close="handleClose" class="billFormDialog"
      ...
      <span slot="title" class="con-title">
        ...
        <span v-if="!dataForm.id">新增销售退库单</span>
        <span v-if="dataForm.id">编辑销售退库单</span>
...
<!--      <div class="association-creation">关联创建</div>-->
      <el-form :model="dataForm" :rules="rules" ref="dataForm">
        <table class="topTable">
          ...
          <tr>
            ...
            <!--</td>-->
            <!--测试-->
            <!--cancellingStocksType-->
            <td>
              <el-form-item label="退库类型:" prop="type" class="el-form-flex">
                <el-select v-model="dataForm.type" size="small" placeholder="">
                  <el-option
          ...
          <tr>
            <td>
              <el-form-item label="退货快递名称:" prop="expressName" class="el-form-flex">
                <el-input v-model="dataForm.expressName" type="textarea" :rows="2" placeholder="请输入退货快递名称"></el-input>
              </el-form-item>
            </td>
            <td>
              <el-form-item label="快递单号:" prop="expressNumber" class="el-form-flex">
                <el-input v-model="dataForm.expressNumber" type="textarea" :rows="2" placeholder="请输入快递单号"></el-input>
              </el-form-item>
            </td>
          </tr>
          <tr>
    ...
    <el-table :data="items" border highlight-current-row width="100%" :height="clientHeight"
      ...
      <el-table-column prop="sourceBillNo" label="关联单据" width="180">
        <template slot-scope="scope">
          <div @click.stop="handleSalesOrderSelector" style="cursor: pointer;">
      ...
      <el-table-column prop="orderBillNo" label="订单编号" width="180" align="center">
        <template slot-scope="scope">
          <el-input size="mini" v-model="scope.row.orderBillNo" readonly="readonly"
                    class="readonly-el-input center-el-input"></el-input>
          <span>{{scope.row.orderBillNo}}</span>
        </template>
      </el-table-column>
      ...
      <el-table-column prop="price" label="单价" width="120" align="right" class-name="col-required">
        <template slot-scope="scope">
          <el-input size="mini" v-model="scope.row.price" readonly="readonly" class="right-el-input readonly-el-input"
      ...
      <el-table-column prop="amount" label="金额" width="120" align="right" class-name="col-required">
        <template slot-scope="scope">
          <el-input size="mini" v-model="scope.row.amount" readonly="readonly" class="right-el-input readonly-el-input"
      ...
      <el-table-column prop="taxRate" label="税率" width="80" align="center">
        <template slot-scope="scope">
          ...
          <span class="readonly-el-input right-el-input">{{scope.row.taxRate}}</span></template>
      </el-table-column>
      ...
      <el-table-column prop="taxPrice" label="含税单价" width="120" align="right" class-name="col-required">
        <template slot-scope="scope">
          <el-input size="mini" v-model="scope.row.taxPrice" readonly="readonly" class="right-el-input readonly-el-input"
      ...
      <el-table-column prop="taxAmount" label="价税合计" width="120" align="right" class-name="col-required">
        <template slot-scope="scope">
          <el-input size="mini" v-model="scope.row.taxAmount" readonly="readonly" class="right-el-input readonly-el-input"
    ...
    <!-- 关联单据 -->
    <sales-stock-out-selector :dialogSelectorVisible="dialogSelectorVisible" v-if="dialogSelectorVisible"
                              ...
                              :type="1">
    </sales-stock-out-selector>
  </el-dialog>
</template>

<script>
  import {getFloat} from '@/utils/util'
  import {addBill, getBill, tempAddBill} from '@/api/erp/wms/bill/salesStockReturn';

  export default {
    name: "salesStockReturnForm",
    components: {
      ...
      'sales-stock-out-selector': () => import('../components/salesStockOutSelector')
    },
    ...
    data() {
      return {
        ...
        dataForm: {
          ...
          type: undefined,
          cancellingStocksType: undefined,
          salerId: undefined,
          salerName: undefined,
          salerCode: undefined,
          currencyId: '1',
    ...
    methods: {
      ...
      handleEdit(index, row, type, val) {
        ...
        if (row.qty && row.qty != '') {
          if (type == 'qty' || type == 'price' || type == 'taxRate') {
            //关联单据时数量不能大于未退库数量
            if (type == 'qty' && row.notStockReturnQty && Number(row.qty) > Number(row.notStockReturnQty)) {
              this.$message.warning('数量不能大于未退库数量[' + row.notStockReturnQty + ']');
              row.qty = row.notStockReturnQty;
            }
      ...
      submitSelectorSuccess(obj) {
        ...
        for (var index in obj.list) {
          ...
          var itemObj = {};
          if(item.sourceBillNo) {
            itemObj.orderBillNo = item.sourceBillNo
          } else {
            itemObj.orderBillNo = item.mainDTO.sourceBillNo
          }
          itemObj.skuNo = item.skuNo;
          ...
          if (obj.sourceObject.key == 'XSCKD' || obj.sourceObject.key == 'ZFCKD') {
            itemObj.qty = item.qty - item.stockReturnQty + '';
            itemObj.amount = getFloat(Number(item.price* (item.qty-item.stockReturnQty)),2);
            itemObj.tax = getFloat(Number(item.price* (item.qty-item.stockReturnQty)*item.taxRate/100),2);
            itemObj.taxAmount = getFloat(Number(item.taxPrice* (item.qty-item.stockReturnQty)),2);
          } else {
            itemObj.qty = item.qty - item.stockReturnSalesQty + '';
            itemObj.amount = getFloat(Number(item.price* (item.qty-item.stockReturnSalesQty)),2);
            itemObj.tax = getFloat(Number(item.price* (item.qty-item.stockReturnSalesQty)*item.taxRate/100),2);
            itemObj.taxAmount = getFloat(Number(item.taxPrice* (item.qty-item.stockReturnSalesQty)),2);
          }
          itemObj.notStockReturnQty = itemObj.qty * 1;
