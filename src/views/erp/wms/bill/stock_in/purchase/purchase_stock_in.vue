<template>
  <div class="full-purchase bill-container">
    ...
    <div style="margin-top: 1px;">
      <el-button type="text" icon="el-icon-plus" v-if="$store.getters.groupNames.indexOf('总经理') > -1 || $store.getters.groupNames.indexOf('仓管') > -1 || $store.getters.groupNames.indexOf('仓库') > -1" @click="handleCreate" id="add_bill">新建</el-button>
    ...
    <el-table :data="list" border style="width: 100%" v-loading="listLoading" ref="dataTable"
      ...
      <el-table-column prop="mainDTO.billNo" align="center" label="单据编号" width="180">
      ...
      <el-table-column prop="mainDTO.supplierName" label="供应商" align="center" width="180">
      </el-table-column>
      ...
      <!--</el-table-column>-->
      <el-table-column prop="mainDTO.warehouseName" label="入库仓库" align="center" width="120">
      </el-table-column>
      ...
      <el-table-column prop="price" label="单价" align="right" width="80" v-if="username == 'admin' || username == 'gxcl' || username == '97012'||$store.getters.groupNames.indexOf('总经理') > -1||$store.getters.groupNames.indexOf('供应链总监') > -1">
        <template slot-scope="scope">
          <span>{{scope.row.price.toFixed(4)}}</span>
        </template>
      </el-table-column>
      <el-table-column prop="amount" label="金额" align="right" width="120" class-name="col-required" v-if="username == 'admin' || username == 'gxcl' || username == '97012'||$store.getters.groupNames.indexOf('总经理') > -1||$store.getters.groupNames.indexOf('供应链总监') > -1">
        <template slot-scope="scope">
          <span>{{scope.row.amount.toFixed(4)}}</span>
        </template>
      </el-table-column>
      <el-table-column prop="tax" label="税额" align="right" width="120" class-name="col-required" v-if="username == 'admin' || username == 'gxcl' || username == '97012'||$store.getters.groupNames.indexOf('总经理') > -1||$store.getters.groupNames.indexOf('供应链总监') > -1">
        <template slot-scope="scope">
          <span>{{scope.row.tax.toFixed(4)}}</span>
        </template>
      </el-table-column>
      <el-table-column prop="taxPrice" label="含税单价" align="right" width="120" v-if="username == 'admin' || username == 'gxcl' || username == '97012'||$store.getters.groupNames.indexOf('总经理') > -1||$store.getters.groupNames.indexOf('供应链总监') > -1">
        <template slot-scope="scope">
          <span>{{scope.row.taxPrice.toFixed(4)}}</span>
        </template>
      </el-table-column>
      <el-table-column prop="taxAmount" label="价税合计" align="right" width="120" class-name="col-required" v-if="username == 'admin' || username == 'gxcl' || username == '97012'||$store.getters.groupNames.indexOf('总经理') > -1||$store.getters.groupNames.indexOf('供应链总监') > -1">
        <template slot-scope="scope">
          <span>{{scope.row.taxAmount.toFixed(4)}}</span>
        </template>
      </el-table-column>
      <el-table-column prop="paymentApplyReturnAmount" label="已申请金额" align="right" width="120" class-name="col-required" v-if="username == 'admin' || username == 'gxcl' || username == '97012'||$store.getters.groupNames.indexOf('总经理') > -1||$store.getters.groupNames.indexOf('供应链总监') > -1">
        <template slot-scope="scope">
          <span>{{scope.row.paymentApplyReturnAmount.toFixed(4)}}</span>
        </template>
      </el-table-column>
      <el-table-column prop="sourceBillNo" label="关联单据" align="center" width="180" class-name="col-required">
      </el-table-column>
    ...
    <el-table :data="list" border style="width: 100%" v-loading="listLoading" ref="dataTable"
      ...
      <el-table-column prop="billNo" label="单据编号" align="center" width="200">
        <template slot-scope="scope">
      ...
      <el-table-column prop="billAmount" label="金额" align="right" width="120" class-name="col-required" v-if="username == 'admin' || username == 'gxcl' || username == '97012'">
        <template slot-scope="scope">
          <span>{{scope.row.billAmount.toFixed(4)}}</span>
        </template>
      </el-table-column>
      <el-table-column prop="taxBillAmount" label="价税合计" align="right" width="120" class-name="col-required" v-if="username == 'admin' || username == 'gxcl' || username == '97012'">
        <template slot-scope="scope">
          <span>{{scope.row.taxBillAmount.toFixed(4)}}</span>
        </template>
      </el-table-column>
      <el-table-column prop="sourceBillNo" label="关联单据" align="center" width="250">
      </el-table-column>
      <el-table-column prop="warehouseName" label="入库仓库" align="center" width="180">
      </el-table-column>
      ...
      <!--</el-table-column>-->
      <el-table-column prop="billDate" label="单据日期" align="center" width="200">
      </el-table-column>
    </el-table>
    ...
    <!-- 新增采购入库弹窗 -->
    <purchase_stock_in_form :dialogFormVisible="dialogFormVisible" v-if="dialogFormVisible" :id="id"
                            @submitSuccess="submitSuccess" @handleClose="handleClose">
    </purchase_stock_in_form>

    <!-- 采购入库详情弹窗 -->
    <purchase_stock_in_view :dialogFormVisible="dialogDetailVisible" v-if="dialogDetailVisible" :id="id"
                            @submitDetailSuccess="submitDetailSuccess" @handleClose="handleDetailClose">
    </purchase_stock_in_view>
  </div>
</template>

<script>
  import {selectPage, selectPageByItem, batchDelBill} from '@/api/erp/wms/bill/purchaseStockIn';
  ...
  export default {
    ...
    components: {
      'purchase_stock_in_form': () => import('./purchase_stock_in_form'),
      'purchase_stock_in_view': () => import('./purchase_stock_in_view')
    },
    data() {
      return {
      ...
      handleDownload() {
        ...
        import('@/vendor/Export2Excel').then(excel => {
          ...
          if (this.choose == false) {
            ...
            tHeader = ['单据日期', '单据编号', '制单人', '采购员', '供应商', '币别', '金额', '价税合计', '关联单据', '入库仓库', '仓位', '备注', '单据状态', '开票状态', '记账状态', '商品名称', '数量', '单位', '单价', '金额', '税额', '含税单价', '价税合计', '备注', '开票数量', '打印次数'];
            filterVal = ['billDate', 'billNo', 'crtUserName', 'purchaserName', 'supplierName', 'currencyId', 'billAmount', 'taxBillAmount', 'sourceBillNo', 'warehouseName', 'positionName', 'note', 'billStatus', 'invoiceStatus', 'voucherStatus', 'skuName', 'qty', 'unit', 'price', 'billAmount', 'tax', 'taxPrice', 'taxBillAmount', 'itemNote', 'purchaseInvoiceQty', 'printCount'];
          }
