<template>
  <el-dialog :visible.sync="dialogFormVisible" width="70%" @close="handleClose" class="billFormDialog billDetailDialog"
    ...
    <el-table :data="items" border highlight-current-row width="100%" :height="clientHeight"
      ...
      <el-table-column prop="price" label="单价" width="120" align="right" class-name="col-required" v-if="username == 'admin' || username == 'gxcl' || username == '97012'">
        <template slot-scope="scope">
          <span>{{scope.row.price.toFixed(4)}}</span>
        </template>
      </el-table-column>
      <el-table-column prop="amount" label="金额" width="120" align="right" class-name="col-required" v-if="username == 'admin' || username == 'gxcl' || username == '97012'">
        <template slot-scope="scope">
          <span>{{scope.row.amount.toFixed(4)}}</span>
        </template>
      </el-table-column>
      <el-table-column prop="tax" label="税额" width="120" align="right" class-name="col-required" v-if="username == 'admin' || username == 'gxcl' || username == '97012'">
        <template slot-scope="scope">
          <span>{{scope.row.tax.toFixed(4)}}</span>
        </template>
      </el-table-column>
      <el-table-column prop="taxPrice" label="含税单价" width="120" align="right" class-name="col-required" v-if="username == 'admin' || username == 'gxcl' || username == '97012'">
        <template slot-scope="scope">
          <span>{{scope.row.taxPrice.toFixed(4)}}</span>
        </template>
      </el-table-column>
      <el-table-column prop="taxAmount" label="价税合计" width="120" align="right" class-name="col-required" v-if="username == 'admin' || username == 'gxcl' || username == '97012'">
        <template slot-scope="scope">
