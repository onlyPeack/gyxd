<template>
  <el-dialog :visible.sync="dialogFormVisible" width="70%" @close="handleClose" class="billFormDialog"
      ...
      <span slot="title" class="con-title">
        ...
        <span v-if="!dataForm.id">新增直发出库单</span>
        <span v-if="dataForm.id">编辑直发出库单</span>
          ...
          <tr>
            <td colspan="2">
              <el-form-item label="收货地址:" prop="linkAddress" class="el-form-flex">
                <el-input type="input" v-model="dataForm.linkAddress" placeholder="请输入收货地址"></el-input>
              </el-form-item>
            </td>
          </tr>
          <!-- <tr>
            ...
            <td>
              <el-form-item label="收款到期日:" prop="deliverDate" class="el-form-flex">
    ...
    <el-table :data="items" border highlight-current-row width="100%" :height="clientHeight"
      ...
      <el-table-column prop="brandName" label="品名" width="180">
      </el-table-column>
      ...
      <el-table-column prop="canGoodsQty" label="可直发数量" width="180">
        <template slot-scope="scope">
...
<script>
  ...
  import {addBill, getBill, tempAddBill} from '@/api/erp/wms/bill/straightStockOut';
  ...
  export default {
    ...
    methods: {
      ...
      handleEdit(index, row, type, val) {
        ...
        if(type == 'qty'){
          if(row.qty >row.canGoodsQty){
            this.vueSet(this.items[index], 'qty', row.canGoodsQty);
          }
          if(row.qty < 1){
            this.vueSet(this.items[index], 'qty', 1);
          }
        }
        if (row.qty && row.qty != '') {
      ...
      submitSelectorSuccess(obj) {
        ...
        for (var index in obj.list) {
          ...
          if (index == 0) {
            _this.dataForm.sourceBillNo = item.salesOrderBillNo;
            _this.sourceBillNoArr = [{billNo: item.salesOrderBillNo}];
            _this.customerArr = [{id: item.customerId, customerName: item.customerName}];
            ...
            _this.supplierArr = [{id: item.supplierId, supplierName: item.supplierName}];
            _this.dataForm.supplierCode = item.supplierCode;
            _this.dataForm.supplierName = item.supplierName;
            _this.dataForm.supplierId = item.supplierId;
          ...
          var itemObj = {};
          itemObj.maxSum =  item.receiptQty == 0 || item.receiptQty =='0' ? item.qty : Number(item.qty)-Number(item.receiptQty);
          itemObj.skuNo = item.skuNo;
          itemObj.itemNo = item.itemNo;
          ...
          itemObj.seriesName = item.seriesName;
          itemObj.unit = item.unit;
          itemObj.lockingOrderNo = obj.list[0].orderSubSn;
          itemObj.price = item.price + '';
          itemObj.skuName = item.skuName;
          itemObj.qty = item.canGoodsQty;
          itemObj.amount = getFloat(Number(item.price* item.canGoodsQty),2);
          itemObj.taxRate = item.taxRate + '';
          itemObj.tax = getFloat(Number(item.price* (item.qty-item.receiptQty-item.stockOutQty)*item.taxRate/100),2);
          itemObj.taxPrice = item.taxPrice + '';
          itemObj.taxAmount = Number(item.taxPrice * itemObj.qty).toFixed(2)
      ...
      checkItemFuc() {
        for (var i in this.items) {
          ...
          if (this.items[i].skuNo && this.items[i].skuNo != '') {
            if (this.items[i].qty > this.items[i].canGoodsQty) {
