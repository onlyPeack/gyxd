  ...
  <div class="full-purchase bill-container">
    ...
    <div class="filter-container">
      <div>
        ...
        <el-input clearable class="filter-item" style="width: 200px;" placeholder="入库仓库"
    ...
    <el-table :data="list" border style="width: 100%" v-loading="listLoading" ref="dataTable"
      ...
      <el-table-column prop="mainDTO.warehouseName" label="入库仓库" width="180">
      </el-table-column>
      <el-table-column prop="positionName" label="入库仓位" align="center" width="180">
      </el-table-column>
      ...
      <el-table-column prop="qty" label="入库数量" align="right" width="100" class-name="col-required">
      </el-table-column>
      <el-table-column prop="unitCost" label="单位成本" align="right" width="100" class-name="col-required">
      </el-table-column>
      <el-table-column prop="cost" label="成本" align="right" width="100" class-name="col-required">
      </el-table-column>
      <el-table-column prop="mainDTO.printCount" label="打印次数" align="right" width="100">
      </el-table-column>
      <el-table-column prop="mainDTO.billDate" label="入库日期" width="150" align="center" sortable>
      </el-table-column>
    </el-table>
    <!--单据-->
    <el-table :data="list" border style="width: 100%" v-loading="listLoading" ref="dataTable"
      ...
      <el-table-column prop="billDate" label="入库日期" align="center" sortable>
      </el-table-column>
    </el-table>
    ...
    <!-- 新增盘盈入库弹窗 -->
    <check_stock_profit_form :dialogFormVisible="dialogFormVisible" v-if="dialogFormVisible" :id="id"
                             @submitSuccess="submitSuccess" @handleClose="handleClose">
    </check_stock_profit_form>

    <!-- 盘盈入库详情弹窗 -->
    <check_stock_profit_view :dialogFormVisible="dialogDetailVisible" v-if="dialogDetailVisible" :id="id"
                             @submitDetailSuccess="submitDetailSuccess" @handleClose="handleDetailClose">
    </check_stock_profit_view>
  </div>
</template>
...
<script>
  ...
  export default {
    name: "checkStockProfit",
    components: {
      'check_stock_profit_form': () => import('./check_stock_profit_form'),
      'check_stock_profit_view': () => import('./check_stock_profit_view')
    },
    data() {
      return {
      ...
      handleDownload() {
        ...
        import('@/vendor/Export2Excel').then(excel => {
          ...
          if (this.choose == false) {
            tHeader = ['入库日期', '单据编号', '制单人', '入库仓库', '单据状态', '打印次数'];
            filterVal = ['billDate', 'billNo', 'crtUserName', 'warehouseName', 'billStatus', 'printCount'];
          } else {
            for (var i in list) {
              Object.assign(list[i], list[i].mainDTO);
              delete list[i].mainDTO;
            }
            tHeader = ['入库日期', '单据编号', '制单人', '入库仓库', '入库仓位', '商品编码', '商品名称', '单位', '入库数量', '单位成本', '成本', '打印次数'];
            filterVal = ['billDate', 'billNo', 'crtUserName', 'warehouseName', 'positionName', 'skuNo', 'skuName', 'unit', 'qty', 'unitCost', 'cost', 'printCount'];
          }
          excel.export_json_to_excel2(tHeader, list, filterVal, '盘盈入库单');
