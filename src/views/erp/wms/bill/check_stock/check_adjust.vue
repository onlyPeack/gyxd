<template>
  <div class="full-purchase bill-container">
    ...
    <el-table :data="list" border style="width: 100%" v-loading="listLoading" ref="dataTable"
              ...
              highlight-current-row element-loading-text="正在查询中。。。" :height="clientHeight" :default-sort="{prop: 'mainDTO.billNo', order: 'descending'}"
      ...
      <el-table-column prop="crtUserNo" label="制单人工号" align="center" width="180">
      </el-table-column>
      <el-table-column prop="crtUserName" label="制单人" align="center" width="180">
      </el-table-column>
      <el-table-column prop="oldAllocationPrice" label="原调拨价" align="center" width="180">
      </el-table-column>
      <el-table-column prop="newAllocationPrice" label="库存调拨价" align="center" width="180">
      </el-table-column>
      <el-table-column prop="newTransferDiscount" label="调拨折扣" align="center" width="180">
      </el-table-column>
      ...
      <el-table-column prop="lockedQty" label="锁库数量" width="200">
      </el-table-column> -->
      <el-table-column prop="warehouseName" label="仓库" width="200">
      </el-table-column>
      <el-table-column prop="positionName" label="仓位" width="200">
      </el-table-column>
      <el-table-column prop="specifications" label="规格型号" width="200">
      </el-table-column>
      <el-table-column prop="mainDTO.billDate" label="出库日期" width="150" align="center" sortable>
      </el-table-column>
    </el-table>
    <!--单据-->
    <el-table :data="list" border style="width: 100%" v-loading="listLoading" ref="dataTable"
              @row-click="handleRowClick" @selection-change="handlechangeFun" size="mini"
              highlight-current-row element-loading-text="正在查询中。。。" :height="clientHeight" :default-sort="{prop: 'billNo', order: 'descending'}"
      ...
      <el-table-column prop="billDate" label="出库日期" align="center" sortable>
      </el-table-column> -->
    </el-table>
    ...
    <!-- 新增盘亏出库弹窗 -->
    <check_adjust_form :dialogFormVisible="dialogFormVisible" v-if="dialogFormVisible" :id="id"
                           @submitSuccess="submitSuccess" @handleClose="handleClose">
    </check_adjust_form>

    <!-- 盘亏出库详情弹窗 -->
    <check_adjust_view :dialogFormVisible="dialogDetailVisible" v-if="dialogDetailVisible" :id="id"
...
<script>
  import {selectPage, selectPageByItem, batchDelBill} from '@/api/erp/wms/bill/checkAdjust';
  import {getFloat} from '@/utils/util';

  export default {
    name: "checkAdjust",
    components: {
      'check_adjust_form': () => import('./check_adjust_form'),
      'check_adjust_view': () => import('./check_adjust_view')
    },
    data() {
      return {
      ...
      handleDownload() {
        ...
        import('@/vendor/Export2Excel').then(excel => {
          ...
          if (this.choose == false) {
            tHeader = ['单据编号', '单据状态', '单据日期', '制单人工号', '制单人'];
            filterVal = ['billNo', 'billStatus', 'billDate', 'crtUserNo', 'crtUserName'];
          } else {
            for (var i in list) {
              Object.assign(list[i], list[i].mainDTO);
              delete list[i].mainDTO;
            }
            tHeader = ['单据编号', '制单人工号', '制单人', '原调拨价','库存调拨价', '调拨折扣','出库仓库',   '商品编码', '商品名称','品牌', '标准系列','返点系列', '订货号','规格型号','出库日期'];
            filterVal = ['billNo', 'crtUserNo', 'crtUserName', 'oldAllocationPrice','newAllocationPrice','newTransferDiscount', 'warehouseName',   'skuNo', 'skuName', 'brandName', 'seriesName','rebateSeries' ,'itemNo','specifications','billDate'];
          }
          excel.export_json_to_excel2(tHeader, list, filterVal, '库存调价单');
      ...
      getListByItem() {
        ...
        selectPageByItem(this.listQuery).then(response => {
          ...
          for (let i = 0; i <this.list.length ; i++) {
            this.list[i].newTransferDiscount=Number(this.list[i].newTransferDiscount).toFixed(4)
          }
          this.total = response.data.total;
          this.listLoading = false;
        }).catch(() => {
