  ...
  <div class="full-purchase bill-container">
    ...
    <div class="filter-container">
      <div>
        ...
        <el-input clearable class="filter-item" style="width: 200px;" placeholder="盘点人工号"
                  v-model="listQuery.crtUserNo"
                  @keyup.enter.native="handleFilter">
        </el-input>
        <el-input clearable class="filter-item" style="width: 200px;" placeholder="盘点人"
                  v-model="listQuery.crtUserName"
                  @keyup.enter.native="handleFilter">
        </el-input>
        <el-input clearable class="filter-item" style="width: 200px;" placeholder="盘点仓库"
    ...
    <el-table :data="list" border style="width: 100%" v-loading="listLoading" ref="dataTable"
              ...
              @row-dblclick="handleOpenDetailOrForm" :default-sort="{prop: 'billDate', order: 'descending'}">
      <el-table-column type="selection" width="60" align="center">
      </el-table-column>
      <el-table-column type="index" align="center">
      </el-table-column>
      ...
      <el-table-column prop="crtUserNo" label="盘点人工号" align="center">
      </el-table-column>
      <el-table-column prop="crtUserName" label="盘点人" align="center">
      </el-table-column>
      <el-table-column prop="warehouseName" label="盘点仓库">
      </el-table-column>
      <el-table-column prop="billDate" label="盘点日期" align="center" sortable>
      </el-table-column>
    ...
    <!-- 新增库存盘点弹窗 -->
    <stock_taking_form :dialogFormVisible="dialogFormVisible" v-if="dialogFormVisible" :id="id"
                       @submitSuccess="submitSuccess" @handleClose="handleClose">
    </stock_taking_form>

    <!-- 库存盘点详情弹窗 -->
    <stock_taking_detail :dialogFormVisible="dialogDetailVisible" v-if="dialogDetailVisible" :id="id"
...
<script>
  ...
  export default {
    name: "stock_taking",
    components: {
      'stock_taking_form': () => import('./stock_taking_form'),
      'stock_taking_detail': () => import('./stock_taking_view')
    },
    data() {
      return {
      ...
      handleDownload() {
        ...
        import('@/vendor/Export2Excel').then(excel => {
          ...
          if (this.choose == false) {
            tHeader = ['盘点日期', '单据编号', '盘点人', '盘点仓库', '单据状态', '打印次数'];
            ...
            tHeader = ['盘点日期', '单据编号', '盘点人', '盘点仓库', '盘点仓位', '商品编码', '商品名称', '单位', '数量', '盘点数量', '打印次数'];
            filterVal = ['billDate', 'billNo', 'crtUserName', 'warehouseName', 'positionName', 'skuNo', 'skuName', 'unit', 'qty', 'takingQty', 'printCount']
          }
          excel.export_json_to_excel2(tHeader, list, filterVal, '库存盘点单');
      ...
      getSummaries(param) {
        ...
        columns.forEach((column, index) => {
          ...
          if (!values.every(value => isNaN(value))) {
            if (column.property == 'takingQty' || column.property == 'qty') {
