<template>
  <el-dialog :visible.sync="dialogFormVisible" width="70%" @close="handleClose" class="billFormDialog"
      ...
      <span slot="title" class="con-title">
        ...
        <span v-if="!dataForm.id">新增库存盘点单</span>
        <span v-if="dataForm.id">编辑库存盘点单</span>
    ...
    <el-table :data="items" border highlight-current-row width="100%" :height="clientHeight"
      ...
      <el-table-column prop="qty" label="库存数量" width="150" align="right" class-name="col-required">
        <template slot-scope="scope">
          <el-input size="mini" v-model="scope.row.qty" class="right-el-input" readonly="readonly"></el-input>
          <span>{{scope.row.qty}}</span>
        </template>
      </el-table-column>
      <el-table-column prop="takingQty" label="盘点数量" width="150" align="right" class-name="col-required">
        <template slot-scope="scope">
          <el-input size="mini" v-model="scope.row.takingQty"></el-input>
    ...
    <!-- 盘点仓库弹窗-->
    <el-dialog title="请选择盘点仓库" :visible.sync="warehouseVisible" append-to-body>
...
<script>
  ...
  import {addBill, getBill, tempAddBill} from '@/api/erp/wms/bill/stockTaking';
      ...
      checkItemFuc() {
        for (var i in this.items) {
          ...
          if (this.items[i].skuNo && this.items[i].skuNo != '') {
            ...
            if (!this.items[i].takingQty || this.items[i].takingQty == '') {
              this.$message.error('第' + (i * 1 + 1) + '行的盘点数量必填项，请填写了再提交');
