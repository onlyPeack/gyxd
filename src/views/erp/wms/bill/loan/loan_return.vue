<template>
  <div class="full-purchase bill-container">
    ...
    <el-table :data="list" border style="width: 100%" v-loading="listLoading" ref="dataTable"
      ...
      <el-table-column prop="mainDTO.billDate" label="归还日期" width="150" sortable>
      </el-table-column>
    </el-table>
    <!--单据-->
    <el-table :data="list" border style="width: 100%" v-loading="listLoading" ref="dataTable"
      ...
      <el-table-column prop="applierName" label="归还人" width="150">
      </el-table-column>
      <el-table-column prop="conDeptName" label="归还部门" width="150">
      </el-table-column>
      <el-table-column prop="printCount" label="打印次数" width="80">
      </el-table-column>
      <el-table-column prop="crtTime" label="归还日期" width="150" sortable>
      </el-table-column>
    </el-table>
    ...
    <!-- 新增归还单弹窗 -->
    <loan_return_form :dialogFormVisible="dialogFormVisible" v-if="dialogFormVisible" :id="id"
                      @submitSuccess="submitSuccess" @handleClose="handleClose">
    </loan_return_form>

    <!-- 归还单详情弹窗 -->
    <loan_return_view :dialogFormVisible="dialogDetailVisible" v-if="dialogDetailVisible" :id="id"
...
<script>
  import {batchDelBill, selectPage, selectPageByItem} from '@/api/erp/wms/bill/returnStock';
  import {getFloat} from '@/utils/util';

  export default {
    name: "loanReturn",
    components: {
      'loan_return_form': () => import('./loan_return_form'),
      'loan_return_view': () => import('./loan_return_view')
    },
    data() {
      return {
      ...
      handleDownload() {
        ...
        import('@/vendor/Export2Excel').then(excel => {
          ...
          if (this.choose == false) {
            tHeader = ['归还日期', '单据编号', '制单人', '归还人', '归还部门', '单据状态', '打印次数'];
            ...
            tHeader = ['归还日期', '单据编号', '制单人', '归还人', '归还部门', '单据状态', '外借仓库', '外借仓位', '商品编码', '商品名称', '单位', '外借数量', '归还数量', '打印次数'];
            filterVal = ['crtTime', 'billNo', 'crtUserName', 'applierName', 'conDeptName', 'billStatus', 'warehouseName', 'positionName', 'skuNo', 'skuName', 'unit', 'qty', 'returnQty', 'printCount'];
          }
          excel.export_json_to_excel2(tHeader, list, filterVal, '归还单');
