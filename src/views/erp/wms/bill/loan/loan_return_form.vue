<template>
  <el-dialog :visible.sync="dialogFormVisible" width="70%" @close="handleClose" class="billFormDialog"
      ...
      <span slot="title" class="con-title">
        ...
        <span v-if="!dataForm.id">新增归还单</span>
        <span v-if="dataForm.id">编辑归还单</span>
    ...
    <div class="center-container">
      ...
      <el-form :model="dataForm" :rules="rules" ref="dataForm">
        <table class="topTable">
          <tr>
            <td width="50%">
              <el-form-item label="归还日期:" prop="billDate" class="el-form-flex">
          ...
          <tr>
            <td>
              <el-form-item label="归还人:" prop="applierName" class="el-form-flex">
            ...
            <td>
              <el-form-item label="归还部门:" prop="conDeptName" class="el-form-flex">
    ...
    <el-table :data="items" border highlight-current-row width="100%" :height="clientHeight"
      ...
      <el-table-column prop="qty" label="外借数量" width="100" align="right" class-name="col-required">
        <template slot-scope="scope">
      ...
      <el-table-column prop="returnQty" label="归还数量(*)" width="100" align="right" class-name="col-required">
        <template slot-scope="scope">
          <el-input size="mini" v-model="scope.row.returnQty" class="right-el-input"></el-input>
    ...
    <!-- 关联单据 -->
    <loan-out-selector :dialogSelectorVisible="dialogSelectorVisible" v-if="dialogSelectorVisible"
...
<script>
  ...
  import {addBill, getBill, tempAddBill} from '@/api/erp/wms/bill/returnStock';
  ...
  export default {
    ...
    components: {
      ...
      'loan-out-selector': () => import('./components/loanOutSelector')
    },
    ...
    methods: {
      ...
      handleValueAdd() {
        var expectedReturnDate = new Date().toLocaleDateString().replace(/\//g, '-');
        const d = {
          date: '',
      ...
      getSummaries(param) {
        ...
        columns.forEach((column, index) => {
          ...
          } else if (column.property === 'returnQty') {
            values = data.map(item => Number(item.returnQty));
          }

          if (column.property === 'qty' || column.property === 'returnQty') {
      ...
      submitSelectorSuccess(obj) {
        ...
        for (var index in obj.list) {
          ...
          if (index == 0) {
            _this.applicantUserArr = item.mainDTO.applicantUserArr;
            _this.dataForm.applierId = item.mainDTO.applierId;
            _this.dataForm.applierName = item.mainDTO.applierName;
            _this.dataForm.conDeptId = item.mainDTO.conDeptId;
            _this.dataForm.conDeptName = item.mainDTO.conDeptName;
      ...
      checkItemFuc() {
        for (var i in this.items) {
          ...
          if (this.items[i].skuNo && this.items[i].skuNo != '') {
            ...
            if (!this.items[i].returnQty || this.items[i].returnQty == '') {
              this.$message.error('第' + (i * 1 + 1) + '行的归还数量必填项，请填写了再提交');
              this.$refs['tableElInput' + (i * this.tableElInputNum + 2)].$el.querySelector('input').focus();
              this.$refs['tableElInput' + (i * this.tableElInputNum + 2)].$el.querySelector('input').select();
              return;
            }
