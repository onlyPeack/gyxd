...
<script>
  import {page} from '@/api/erp/crm/customerChance';
  ...
  export default {
    ...
    methods: {
      ...
      getRowData(val) {
        if (val) {
          if (this.isSingle) {
            this.chooseCustomer = [val.name];
            this.currentCustomer = [val];
          } else {
            for (var i in this.chooseCustomer) {
              if (this.chooseCustomer[i] == val.name) {
                return;
              }
            }
            this.chooseCustomer.push(val.name);
            this.currentCustomer.push(val);
          }
        }
      },
      onSubmit(row) {
        this.$emit('closeChanceDialog', this.currentCustomer)
      },
