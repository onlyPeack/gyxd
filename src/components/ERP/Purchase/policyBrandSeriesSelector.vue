<template>
  <el-dialog :visible.sync="dialogVisible" width="70%" @close="handleClose" :close-on-click-modal="false"
             :before-close="handleClose" title="选择品牌返点系列" append-to-body class="public-dialog">
    <div class="app-container calendar-list-container">
      <!-- 查询和其他操作 -->
      <div class="filter-container">
        <el-input clearable class="filter-item" style="width: 200px;" placeholder="请输入品牌名称"
                  v-model="listQuery.brand"></el-input>
        <el-input clearable class="filter-item" style="width: 200px;" placeholder="请输入返点系列名称"
                  v-model="listQuery.companySeries"></el-input>
        <el-input clearable class="filter-item" style="width: 200px;" placeholder="请输入渠道类型名称"
                  v-model="listQuery.channelTypeName"></el-input>
        <el-button class="filter-item" type="primary" icon="el-icon-search" @click="handleFilter">搜索</el-button>
        <el-button class="filter-item" icon="el-icon-delete" @click="clear" style="margin-left: 10px;">重置</el-button>
      </div>

      <!-- 查询结果 -->
      <el-table size="small" :data="list" v-loading="listLoading" element-loading-text="正在查询中。。。" border fit @row-dblclick="rowClick"
...
<script>
  ...
  export default {
    name: 'policyBrandSeriesSelector',
    props: ['dialogVisible','brand'],
    data() {
      return {
        list: undefined,
        total: undefined,
    ...
    created() {
      ...
      if(this.brand) {
        console.log('有品牌');
        this.listQuery.brand = this.brand
      }
    },
    methods: {
      getList() {
