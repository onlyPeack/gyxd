      ...
      <el-col :span="6">
        <el-row>
          <el-col :span="24" style="line-height: 36px;padding-left: 20px;">
            已选人员：
            <br/>
            <el-tag v-show="chooseAdmin.length>0" :key="tag" v-for="(tag,index) in chooseAdmin" :closable="false"
                    :disable-transitions="true" style="margin-right: 15px;">
              {{tag}}
            </el-tag>
          </el-col>
        </el-row>
...
<script>
  ...
  export default {
    methods: {
      ...
      onSubmit() {
        this.$emit('closeStaffUserDialog', this.currentUser)
      },
      reset() {
        this.chooseAdmin = [];
        this.currentUser = [];
